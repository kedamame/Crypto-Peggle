...
const MBC_FORCE        = 1.35;  // magnetar-birth chirp outward at core (decays t*t)
const MBC_RANGE        = 155;   // birth-chirp flare radius px
const MBC_RELEASE      = 8;     // frames each chirp pulse pushes + twists
const MBC_WARN         = 48;    // telegraph frames (rings densify with chirpN)
const MBC_TWIST0       = 0.07;  // base velocity twist rad on first pulse
const MBC_TWIST_STEP   = 0.028; // twist grows each subsequent pulse (Lense-Thirring chirp)
const MBC_TWIST_CAP    = 0.24;  // hard cap so stacks stay escapable
const RMF_HALF         = 24;    // RM-flare corridor half-width px
const RMF_ANGLE        = 0.18;  // one-shot velocity twist (rad) when band crosses a ball
const RMF_SPD          = 4.5;   // corridor sweep speed px/f
const RMF_WARN         = 40;    // telegraph frames before a sweep
const DDS_R_MIN         = 40;    // dimming DM shell min radius px
const DDS_R_MAX         = 180;   // dimming DM shell max radius px
const DDS_GROW_FRAMES   = 720;   // frames to expand min→max
const DDS_BAND_HALF     = 16;    // shell band half-width px
const DDS_DRAG          = 0.988; // in-band speed soft drag
const DDS_REFRACT       = 0.12;  // weak outward normal nudge
const DDS_FADE_DUR      = 24;
const DDS_RECONDENSE_DUR = 24;
const DDS_LIT_DUR       = 10;
const DDS_LIT_BINS      = 12;
const PRS_R             = 90;    // persistent radio nebula radius px
const PRS_DRAG          = 0.988; // isotropic in-nebula drag (angle preserved)
const PRS_OUT           = 0.05;  // weak outward push so the core cannot trap
const PRS_MIN_SPD       = BALL_SPEED * 0.40;
const GWBLSS_ACCEL      = 0.018; // GWB-LSS anisotropic bias peak accel along fixed axis
const ULDM_AMP          = 0.0022; // ultralight DM undulation twist amplitude (rad)
const ULDM_OMEGA        = 0.045;  // ultralight DM undulation angular frequency
const SFE_RX            = 100;   // JWST SFE overproduction ellipse long radius
const SFE_RY            = 70;    // short radius
const SFE_BOOST         = 1.008; // isotropic speed boost inside ellipse
const SFE_OUT           = 0.04;  // weak outward so overproduction cannot trap
const SFE_MIN_SPD       = BALL_SPEED * 0.40;
const DEPERT_LEN         = 200;
const DEPERT_THICK       = 48;
const DEPERT_GRAV        = 0.90;
const DEPERT_TWIST       = 0.0015;
const NEGNU_BOOST        = 1.0035; // effective -m_nu toy: mild isotropic growth
const FRBD_WARN          = 120;  // long telegraph before delayed FRB-like burst
const FRBD_ANGLE         = 0.16; // milder board-wide twist than FRB
const FRBD_RING_RANGE    = 110;
const CURVK_BAND         = 28;   // spatial curvature keel edge band width
const CURVK_RATE         = 0.01; // rad/f heading bias toward/away from center
...
const DPBH_HALO_R        = 110;   // dressed PBH DM-halo pull radius
const DPBH_HALO_F        = 0.22;  // dressed PBH halo pull
const DPBH_RING_R        = 48;    // microlens caustic ring radius
const DPBH_RING_HALF     = 10;    // caustic band half-width
const DPBH_KICK          = 0.55;  // radial kick on ring cross
const DPBH_TWIST         = 0.12;  // velocity twist on ring cross
...
const SCPT_HALF         = 6;     // wall band half-width
...
const BARYSOFT_RX        = 100;
const BARYSOFT_RY        = 70;
const BARYSOFT_DRAG      = 0.985; // per-frame speed softener (heading kept)
const CHAM_RANGE         = 110;
const CHAM_FORCE         = 0.22;
const CHAM_NEIGHBOR      = 80;    // peg count radius for screening
const CHAM_SCREEN        = 0.7;   // f /= (1 + SCREEN * n)
const LYAG_R             = 95;    // visual BAO ghost ring radius
const LYAG_OFFSET        = 8;     // |r_phys - r_vis|
const LYAG_BAND          = 14;    // force band half-width
const LYAG_FORCE         = 0.16;
const FLEXHUM_AMP        = 0.003;
const FLEXHUM_W0         = 0.03;
const FLEXHUM_W1         = 0.05;
const FLEXHUM_W2         = 0.07;
...
function reviveHazardWeakFields(g: GameState) {
  ...
  if (g.axionBirePatchwork) g.axionBirePatchwork.lastStrip = new WeakMap();
...
// Magnetar birth chirp (lv242+): Lense-Thirring newborn magnetar - each flare's twist chirps up.
// chirpN counts completed pulses this level so twist angle grows (Nature 2026 SN 2024afav).
interface BirthChirp { x: number; y: number; period: number; timer: number; releaseTimer: number; chirpN: number; twistSign: 1 | -1 }
// RM flare corridor (lv245+): diagonal LOS band that sweeps; one speed-preserving twist per crossing.
interface RmFlare { tx: number; ty: number; pos: number; timer: number; period: number; twistSign: 1 | -1; hit: WeakSet<object> }
// Dimming DM shell (lv248+): expanding SNR-like shell — in-band micro-drag + weak outward refract.
interface DimDmShell { x: number; y: number; radius: number; phase: 'grow' | 'fadeOut' | 'recondense'; timer: number; litBins: number[] }
// Persistent radio nebula (lv251+): fixed soft halo — isotropic drag + tiny outward push.
interface PrsNebula { x: number; y: number }
// GWB-LSS anisotropic bias (lv254+): board-wide push along a fixed axis, strength from heading quadrupole.
interface GwbLssBias { theta0: number; accel: number }
// JWST SFE overproduction (lv262+): ellipse of mild speed boost + weak outward.
interface SfeOver { x: number; y: number; rx: number; ry: number; axis: number }
// DE perturbation curtain (lv265+): OBB softens gravity + micro twist.
...
interface CosmicShear { x: number; y: number; rx: number; ry: number; axis: number; dots: CosmicShearDot[]; orthoFlip?: boolean }
...
interface CollisionlessShock {
  ...
  inverted?: boolean; // lv76+ rare: V opens forward (upward-ish sweep)
}
...
// FRB old-population delayed burst (lv271+): same twist tell, much longer warn.
interface FrbDelay { x: number; y: number; period: number; timer: number; fireAngle: number; fired: boolean; burstAge: number }
// Spatial curvature keel (lv274+): edge band steers heading open(+)/closed(-).
interface CurvKeel { sign: 1 | -1 }
// Interacting HDE transfer belt (lv277+): one side push, other side drag.
interface IhdeBelt { y: number; halfW: number; dir: 1 | -1 }
...
interface QuadGhostLens { x: number; y: number; flashTimer: number; track: WeakMap<Ball, { minDist: number; inside: boolean }> }
// Free-streaming cutoff blade (lv311+): sweeping thin band that deletes small normal velocity.
interface FsCutoffBlade { horizontal: boolean; pos: number; dir: 1 | -1; passingBalls: WeakSet<Ball> }
// Measurement-disagreement dual field (lv317+): ellipse where shear-twist and clustering-push take turns.
interface MeasDisagreeDual { x: number; y: number; rx: number; ry: number; axis: number }
...
interface HyperStar { x: number; y: number; vx: number; vy: number; respawnTimer: number; warnFromLeft: boolean; warnY: number; returns?: boolean; returned?: boolean }
...
interface BulletCluster { x: number; vx: number; hitCool: number; hitFlash: number; hitX: number; hitY: number; respawnTimer: number; warnFromLeft: boolean; warnY: number; dmContra?: boolean }
...
interface BaryonOscillation { x: number; y: number; litBins: number[][]; phaseInvert?: boolean }
...
interface CosmicBirefringence { x: number; y: number; angle: number; hitFlash: number; hitX: number; hitY: number; hitAngle: number; inverted?: boolean }
...
// Baryonic feedback softening (lv225+): continuous speed damp inside an ellipse.
interface BarySoft {
  x: number; y: number;
  rx: number; ry: number;
  axis: number;
}
// Chameleon screening field (lv228+): force weakens near peg density.
interface ChameleonField {
  x: number; y: number;
  sign: 1 | -1; // outward (+) or inward (-)
}
// Lyα BAO ghost ring (lv234+): visual radius differs from physical force ring.
interface LyaGhostRing {
  x: number; y: number;
  rVis: number;
  rPhys: number;
}
...
interface RogueBH { cx0: number; cy0: number; flashTimer: number; fast?: boolean }
...
interface GameState {
  ...
  gwBackgroundAntiPhase: boolean; // lv74+ rare: diagonal corners blink out of phase
  ...
  barySofts: BarySoft[]; // lv225+ baryonic feedback softening (exclusive with mBias / silk / ia)
  chameleons: ChameleonField[]; // lv228+ chameleon screening (exclusive with ideSiphon / darkHalo / sidm)
  isoBireActive: boolean; // lv231+ isotropic cosmic birefringence drift
  isoBireBeta: number; // +ISOBIRE_BETA or -ISOBIRE_BETA
  lyaGhosts: LyaGhostRing[]; // lv234+ Lyα BAO ghost ring
  flexHumActive: boolean; // lv237+ flexknot GWB hum
  uldmWaveActive: boolean; // lv257+ ultralight DM undulation
  birthChirps: BirthChirp[]; // lv242+ magnetar birth chirp
  rmFlares: RmFlare[]; // lv245+ RM flare corridor
  dimDmShells: DimDmShell[]; // lv248+ dimming DM shell
  prsNebulae: PrsNebula[]; // lv251+ persistent radio nebula
  gwbLssBiases: GwbLssBias[]; // lv254+ GWB-LSS anisotropic bias
  sfeOvers: SfeOver[]; // lv262+ JWST SFE overproduction
  dePertCurtains: DePertCurtain[]; // lv265+ DE perturbation curtain
  negNuFlowActive: boolean; // lv268+ effective negative neutrino mass flow
  frbDelays: FrbDelay[]; // lv271+ FRB old-population delayed burst
  curvKeels: CurvKeel[]; // lv274+ spatial curvature keel
  ihdeBelts: IhdeBelt[]; // lv277+ interacting HDE transfer belt
  memoryBurdenEmbers: MemoryBurdenEmber[]; // lv282+ memory-burden PBH ember
  nakedLrdSeeds: NakedLrdSeed[]; // lv285+ hostless LRD seed
  dressedPbhs: DressedPbh[]; // lv288+ dressed PBH microlens cloak
  fapLooms: FapLoom[]; // lv291+ F_AP anisotropy loom
  ptaCw: PtaContinuousWave | null; // lv294+ resolvable PTA continuous wave
  scptWalls: ScptWall[]; // lv297+ supercooled phase-transition wall
  axionBirePatchwork: AxionBirePatchwork | null; // lv302+ axion-string birefringence patchwork
  quadGhostLenses: QuadGhostLens[]; // lv305+ quadruple-image ghost lens
  edeLawActive: boolean; // lv308+ EDE law blink present on level
  edeLawTimer: number; // countdown to next blink cycle
  edeLawWarn: number; // telegraph remaining
  edeLawBlink: number; // active force-sign-flip remaining
  fsCutoffBlade: FsCutoffBlade | null; // lv311+ free-streaming cutoff blade
  ebParityActive: boolean; // lv314+ EB parity fault
  measDisagreeDuals: MeasDisagreeDual[]; // lv317+ measurement-disagreement dual field
  ...
  cmeTilt: number;      // lv30+ rare: oblique sweep tilt rad (0 = flat)
  reionActive: boolean; // this level has a reionization ionization front
  reionPeriod: number;
  reionTilt: number; // lv79+ rare: oblique front tilt rad
  reionTimer: number;
  ...
  paperMood: PaperMoodState; // subtle cream-board grain mood for this level
...
// ─── Paper moods (wordless cream-board grain; never recolors the paper) ───────
// One primary mood per level (lv40+) plus a rare overlay. Draw/drift only — no physics.
type PaperMoodId =
  | 'none'
...
function emptyPaperMood(): PaperMoodState {
  return { id: 'none', rare: 'none', seamAngle: 0, ashSide: 1, param: 0.5, rareTimer: 0 };
}

/** Deterministic per-level mood (does not consume the layout rng stream). */
function rollPaperMood(level: number): PaperMoodState {
  if (level < 40) return emptyPaperMood();
  const rng = makeRng((((level * 2654435761) ^ 0xc0ffee) >>> 0));
  const pool = paperMoodPool(level);
  if (pool.length === 0) return emptyPaperMood();
  const id = pool[Math.floor(rng() * pool.length)];
  const seamAngle = rng() * Math.PI;
  const ashSide: -1 | 1 = rng() < 0.5 ? -1 : 1;
  const param = 0.25 + rng() * 0.55;
  let rare: PaperMoodRareId = 'none';
  let rareTimer = 0;
  const rares = paperMoodRarePool(level);
  if (rares.length > 0 && rng() < PAPER_MOOD_RARE_P) {
...
function generateLevel(W: number, H: number, launcherY: number, rng: () => number, level = 1): { pegs: Peg[], orangeTotal: number, bumpers: Bumper[], gravZones: GravZone[], wormholes: Wormhole[], wallSegments: WallSegment[], boss: Boss | null, comets: Comet[], lenses: Lens[], cme: { active: boolean; period: number; tilt: number }, pulsars: Pulsar[], gravWaves: GravWave[], vacuums: VacuumBubble[], whiteHoles: WhiteHole[], magnetars: Magnetar[], roguePlanets: RoguePlanet[], quasarJets: QuasarJet[], microBHs: MicroBH[], darkHalos: DarkHalo[], ergospheres: Ergosphere[], magReconnections: MagReconnection[], preSupernovae: PreSupernova[], tidalStretches: TidalStretch[], tachyonStreams: TachyonStream[], cosmicVoids: CosmicVoid[], cosmicShears: CosmicShear[], collisionlessShocks: CollisionlessShock[], silkDampingClouds: SilkDampingCloud[], planckGratings: PlanckDiffractionGrating[], vacuumCherenkovDomains: VacuumCherenkovDomain[], closedTimelikeCurves: ClosedTimelikeCurve[], gravitationalCaustics: GravitationalCaustic[], neutrinoOscillations: NeutrinoOscillation[], gravWaveMemories: GravWaveMemory[], einsteinCrosses: EinsteinCross[], quantumZenoSectors: QuantumZenoSector[], chirpBinary: TransSolarChirp | null, fuzzySolitons: FuzzySoliton[], axionMicrolenses: AxionMicrolens[], holographicRGSheets: HolographicRGSheet[], axionWalls: AxionWall[], frbSources: FRBSource[], antimatterFlecks: AntimatterFleck[], quantumBarriers: QuantumBarrier[], timeDilations: TimeDilation[], cosmicStrings: CosmicString[], darkEnergyPatches: DarkEnergyPatch[], galacticTidalStreams: GalacticTidalStream[], einsteinMirrorRings: EinsteinMirrorRing[], nakedSingularities: NakedSingularity[], hyperStars: HyperStar[], rogueBHs: RogueBH[], oddRadioCircles: OddRadioCircle[], tidalDisruptions: TidalDisruption[], greatAttractor: GreatAttractor | null, bulletClusters: BulletCluster[], baryonOscillations: BaryonOscillation[], laniakeaBasins: LaniakeaBasin[], gwBackgroundActive: boolean, gwBackgroundAntiPhase: boolean, horizonEntropyActive: boolean, entropicDragActive: boolean, pop31Flash: Pop31Flash | null, runawaySMBHs: RunawaySMBH[], phantomMembranes: PhantomMembrane[], alensActive: boolean, bigRings: BigRing[], kszPatches: KszPatch[], subsolarPbhEcho: SubsolarPbhEcho | null, quintomBreathActive: boolean, bhStarCocoons: BhStarCocoon[], dualH0Seam: DualH0Seam | null, hdHumActive: boolean, sidmSpike: SidmSpike | null, nuNullBands: NuNullBand[], tcDmHalos: TcDmHalo[], fsSoftFields: FsSoftField[], ommCores: OmmCore[], frbMicrolenses: FrbMicrolens[], pmfClumps: PmfClump[], ideSiphonBands: IdeSiphonBand[], vacLeaks: VacLeak[], gravEcho: GravEcho | null, momCoupActive: boolean, bosonCaustics: BosonCaustic[], iaContams: IaContam[], signIdeSeams: SignIdeSeam[], phantomBelts: PhantomBelt[], mBiasVeils: MBiasVeil[], varCoupActive: boolean, photoZGates: PhotoZGate[], blueHumActive: boolean, s8Seams: S8Seam[], barySofts: BarySoft[], chameleons: ChameleonField[], isoBireActive: boolean, isoBireBeta: number, lyaGhosts: LyaGhostRing[], flexHumActive: boolean, uldmWaveActive: boolean, birthChirps: BirthChirp[], rmFlares: RmFlare[], dimDmShells: DimDmShell[], prsNebulae: PrsNebula[], gwbLssBiases: GwbLssBias[], sfeOvers: SfeOver[], dePertCurtains: DePertCurtain[], negNuFlowActive: boolean, frbDelays: FrbDelay[], curvKeels: CurvKeel[], ihdeBelts: IhdeBelt[], memoryBurdenEmbers: MemoryBurdenEmber[], nakedLrdSeeds: NakedLrdSeed[], dressedPbhs: DressedPbh[], fapLooms: FapLoom[], ptaCw: PtaContinuousWave | null, scptWalls: ScptWall[], axionBirePatchwork: AxionBirePatchwork | null, quadGhostLenses: QuadGhostLens[], edeLawActive: boolean, fsCutoffBlade: FsCutoffBlade | null, ebParityActive: boolean, measDisagreeDuals: MeasDisagreeDual[], cosmicBirefringences: CosmicBirefringence[], littleRedDots: LittleRedDot[], primordialBHs: PrimordialBH[], darkStars: DarkStar[], cmbAnisotropy: CmbAnisotropy | null, hawkingPoints: HawkingPoint[], quantumFoams: QuantumFoam[], firewalls: Firewall[], superradiances: Superradiance[], negMassBlobs: NegMassBlob[], bubbleUniverses: BubbleUniverse[], bigRip: BigRip | null, cccBoundary: CccBoundary | null, theNothings: TheNothing[], anomalyKind: AnomalyKind | null, reion: { active: boolean; period: number; timer: number; tilt: number } } {
  ...
  if (level >= 20 && (hazChance(hazardRng, 0.5, 20, level) || level === 20)) {
    ...
    if (level >= 30 && hazChance(hazardRng, 0.2)) {
      cme.tilt = (hazChance(hazardRng, 0.5) ? 1 : -1) * (18 * Math.PI / 180);
    }
  }
  ...
  if (level >= 66 && hazChance(hvsRng, 0.45, 66, level)) {
    hyperStars.push({
      ...
      // Returning HVS rare (lv75+, 20%): one reverse re-entry like the red comet.
      returns: level >= 75 && hazChance(hvsRng, 0.2),
    });
  }
  ...
  if (level >= 69 && hazChance(rbhRng, 0.45, 69, level)) {
    rogueBHs.push({
      ...
      flashTimer: 0,
      // Fast Lissajous rare (lv78+, 20%): frequencies ×1.6.
      fast: level >= 78 && hazChance(rbhRng, 0.2),
    });
  }
  ...
  if (level >= 64 && hazChance(bcRng, 0.45, 64, level)) {
    bulletClusters.push({
      ...
      // Contra-DM rare (lv73+, 20%): DM lead trails behind the gas instead of ahead.
      dmContra: level >= 73 && hazChance(bcRng, 0.2),
    });
  }
  ...
  if (level >= 63 && hazChance(baoRng, 0.45, 63, level)) {
    baryonOscillations.push({
      ...
      // Phase-invert rare (lv72+, 20%): radial pull sign flips on a slow sine.
      phaseInvert: level >= 72 && hazChance(baoRng, 0.2),
    });
  }
  ...
  if (level >= 61 && hazChance(lbRng, 0.45, 61, level)) {
    ...
    // Dual-sink rare (lv70+, 20%): second basin on the opposite wall (band force only).
    if (level >= 70 && hazChance(lbRng, 0.2)) {
      let sink2X: number, sink2Y: number;
      if (side === 0)      { sink2X = W * (0.2 + lbRng() * 0.6); sink2Y = topPad + playH; }
      else if (side === 1) { sink2X = 0; sink2Y = topPad + playH * (0.2 + lbRng() * 0.6); }
      else if (side === 2) { sink2X = W * (0.2 + lbRng() * 0.6); sink2Y = topPad; }
      else                 { sink2X = W; sink2Y = topPad + playH * (0.2 + lbRng() * 0.6); }
      const streams2: LaniakeaStream[] = [];
      for (let s = 0; s < 2; s++) {
        const startAngle = (s / 2) * Math.PI + lbRng() * 0.5 + Math.PI * 0.25;
        const startR = Math.min(W, playH) * (0.50 + lbRng() * 0.25);
        const startX = cx + Math.cos(startAngle) * startR;
        const startY = cy + Math.sin(startAngle) * startR;
        const mdx = sink2X - startX, mdy = sink2Y - startY;
        const mlen = Math.sqrt(mdx * mdx + mdy * mdy) || 1;
        const nx = -mdy / mlen, ny = mdx / mlen;
        const curveMag = (50 + lbRng() * 50) * (hazChance(lbRng, 0.5) ? 1 : -1);
        const ctrlX = (startX + sink2X) / 2 + nx * curveMag;
        const ctrlY = (startY + sink2Y) / 2 + ny * curveMag;
        const pts: { x: number; y: number }[] = [];
        for (let i = 0; i < LB_STREAM_PTS; i++) {
          const t = i / (LB_STREAM_PTS - 1);
          const omt = 1 - t;
          pts.push({
            x: omt * omt * startX + 2 * omt * t * ctrlX + t * t * sink2X,
            y: omt * omt * startY + 2 * omt * t * ctrlY + t * t * sink2Y,
          });
        }
        let len = 0;
        for (let i = 0; i < pts.length - 1; i++) {
        ...
        streams2.push({ pts, len });
      }
      laniakeaBasins.push({ sinkX: sink2X, sinkY: sink2Y, streams: streams2 });
    }
  }
  ...
  // Anti-phase corner rare (lv74+, 20%): diagonal corners blink out of phase.
  let gwBackgroundAntiPhase = gwBackgroundActive && level >= 74 && hazChance(gwbRng, 0.2);
  ...
  if (level >= 65 && hazChance(cbRng, 0.40, 65, level)) {
    cosmicBirefringences.push({
      ...
      // Inverted birefringence rare (lv74+, 20%): twist sign flipped.
      inverted: level >= 74 && hazChance(cbRng, 0.2),
    });
  }
  ...
  if (level >= 68 && !hasRedCometLRD && hazChance(lrdRng, 0.45, 68, level)) {
    ...
    // Synced LRD rare (lv77+, 20%): all dots share one blink phase.
    const synced = level >= 77 && hazChance(lrdRng, 0.2);
    const sharedPhase = Math.floor(lrdRng() * (LRD_ON_FRAMES + LRD_OFF_FRAMES));
    for (let i = 0; i < lrdCount; i++) {
      littleRedDots.push({
        x: W * (0.15 + lrdRng() * 0.7),
        y: topPad + playH * (0.15 + lrdRng() * 0.7),
        phase: synced ? sharedPhase : Math.floor(lrdRng() * (LRD_ON_FRAMES + LRD_OFF_FRAMES)),
        hitCool: 0, hitFlash: 0, hitX: 0, hitY: 0,
      });
    }
  }
  ...
  const barySofts: BarySoft[] = [];
  const chameleons: ChameleonField[] = [];
  let isoBireActive = false;
  let isoBireBeta = ISOBIRE_BETA;
  const lyaGhosts: LyaGhostRing[] = [];
  let flexHumActive = false;
  let uldmWaveActive = false;
  const birthChirps: BirthChirp[] = [];
  const rmFlares: RmFlare[] = [];
  const dimDmShells: DimDmShell[] = [];
  const prsNebulae: PrsNebula[] = [];
  const gwbLssBiases: GwbLssBias[] = [];
  const sfeOvers: SfeOver[] = [];
  const dePertCurtains: DePertCurtain[] = [];
  let negNuFlowActive = false;
  const frbDelays: FrbDelay[] = [];
  const curvKeels: CurvKeel[] = [];
  const ihdeBelts: IhdeBelt[] = [];
  const memoryBurdenEmbers: MemoryBurdenEmber[] = [];
  const nakedLrdSeeds: NakedLrdSeed[] = [];
  const dressedPbhs: DressedPbh[] = [];
  const fapLooms: FapLoom[] = [];
  let ptaCw: PtaContinuousWave | null = null;
  const scptWalls: ScptWall[] = [];
  let axionBirePatchwork: AxionBirePatchwork | null = null;
  const quadGhostLenses: QuadGhostLens[] = [];
  let edeLawActive = false;
  let fsCutoffBlade: FsCutoffBlade | null = null;
  let ebParityActive = false;
  const measDisagreeDuals: MeasDisagreeDual[] = [];
  ...
  let reion = { active: false, period: 0, timer: 0, tilt: 0 };
  ...
  if (specialKind(level) === 'special') {
    ...
    for (const arr of [gravZones, wormholes, comets, lenses, pulsars, gravWaves, vacuums,
      ...
      darkStars, hawkingPoints, memoryBurdenEmbers, nakedLrdSeeds, dressedPbhs, fapLooms, ptaCw, scptWalls, axionBirePatchwork, quadGhostLenses, edeLawActive, fsCutoffBlade, ebParityActive, measDisagreeDuals, quantumFoams, firewalls, superradiances, negMassBlobs,
    ...
    gwBackgroundActive = false; gwBackgroundAntiPhase = false; chirpBinary = null; horizonEntropyActive = false; entropicDragActive = false; pop31Flash = null; alensActive = false;
    ...
    frbMicrolenses.length = 0; pmfClumps.length = 0; ideSiphonBands.length = 0; vacLeaks.length = 0; gravEcho = null; bosonCaustics.length = 0; iaContams.length = 0; signIdeSeams.length = 0; phantomBelts.length = 0; mBiasVeils.length = 0; photoZGates.length = 0; s8Seams.length = 0; barySofts.length = 0; chameleons.length = 0; lyaGhosts.length = 0; birthChirps.length = 0; rmFlares.length = 0; dimDmShells.length = 0; prsNebulae.length = 0; gwbLssBiases.length = 0; sfeOvers.length = 0; dePertCurtains.length = 0; frbDelays.length = 0; curvKeels.length = 0; ihdeBelts.length = 0; memoryBurdenEmbers.length = 0; nakedLrdSeeds.length = 0; dressedPbhs.length = 0; fapLooms.length = 0; ptaCw = null; scptWalls.length = 0; axionBirePatchwork = null; quadGhostLenses.length = 0; edeLawActive = false; fsCutoffBlade = null; ebParityActive = false; measDisagreeDuals.length = 0;
  ...
  if (anomalyKind === null && level >= 62 && hazChance(cshRng, 0.45, 62, level)) {
    ...
    cosmicShears.push({
      ...
      axis: cshRng() * Math.PI,
      dots,
      // Ortho-flip rare (lv71+, 20%): shear axis jumps 90° on a slow period.
      orthoFlip: level >= 71 && hazChance(cshRng, 0.2),
    });
  }
  ...
  if (anomalyKind === null && level >= 67 && hazChance(clsRng, 0.40, 67, level)) {
    collisionlessShocks.push({
      ...
      passingBalls: new WeakSet<Ball>(),
      // Inverted-V rare (lv76+, 20%): arms open forward instead of trailing.
      inverted: level >= 76 && hazChance(clsRng, 0.2),
    });
  }
  ...
  if (anomalyKind === null && level >= 72 && hazChance(silkRng, 0.40, 72, level)) {
    ...
    silkDampingClouds.push({
      ...
      dots,
      // Long-axis silk rare (lv80+, 20%): damp along the major axis instead.
      longAxis: level >= 80 && hazChance(silkRng, 0.2),
    });
  }
  ...
  if (anomalyKind === null && level >= 65 && hazChance(causticRng, 0.40, 65, level)) {
    ...
    // Dual caustic rare (lv75+, 20%): second fold line crossing the first at mid.
    if (level >= 75 && hazChance(causticRng, 0.2)) {
      const angle2 = angle + Math.PI * 0.5;
      const halfLen2 = W * 0.22;
      const bend2 = (causticRng() - 0.5) * 70;
      const ca2 = Math.cos(angle2), sa2 = Math.sin(angle2);
      const q0x = midX - ca2 * halfLen2, q0y = midY - sa2 * halfLen2;
      const q2x = midX + ca2 * halfLen2, q2y = midY + sa2 * halfLen2;
      const q1x = midX - sa2 * bend2, q1y = midY + ca2 * bend2;
      const pts2: { x: number; y: number }[] = [];
      for (let i = 0; i < CAUSTIC_PTS; i++) {
        const t = i / (CAUSTIC_PTS - 1);
        const u = 1 - t;
        pts2.push({
          x: u * u * q0x + 2 * u * t * q1x + t * t * q2x,
          y: u * u * q0y + 2 * u * t * q1y + t * t * q2y,
        });
      }
      gravitationalCaustics.push({
        pts: pts2,
  ...
  if (anomalyKind === null && level >= 71 && hazChance(reionRng, 0.40, 71, level)) {
    reion = {
      ...
      // Oblique reion rare (lv79+, 20%): CME-style ±14° tilt.
      tilt: level >= 79 && hazChance(reionRng, 0.2)
        ? (hazChance(reionRng, 0.5) ? 1 : -1) * (14 * Math.PI / 180)
        : 0,
    };
  }
  ...
  // Baryonic feedback softening (lv225+): continuous speed damp inside an ellipse.
  // Exclusive with mBias / silk / iaContams.
  const barySoftRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 225 &&
    mBiasVeils.length === 0 &&
    silkDampingClouds.length === 0 &&
    iaContams.length === 0 &&
    hazChance(barySoftRng, 0.35, 225, level)
  ) {
    barySofts.push({
      x: W * (0.28 + barySoftRng() * 0.44),
      y: topPad + playH * (0.28 + barySoftRng() * 0.44),
      rx: BARYSOFT_RX,
      ry: BARYSOFT_RY,
      axis: barySoftRng() * Math.PI,
    });
  }

  // Chameleon screening field (lv228+): force screened by nearby peg density.
  // Exclusive with ideSiphon / darkHalos / sidmSpike.
  const chamRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 228 &&
    ideSiphonBands.length === 0 &&
    darkHalos.length === 0 &&
    sidmSpike === null &&
    hazChance(chamRng, 0.35, 228, level)
  ) {
    chameleons.push({
      x: W * (0.28 + chamRng() * 0.44),
      y: topPad + playH * (0.28 + chamRng() * 0.44),
      sign: chamRng() < 0.5 ? 1 : -1,
    });
  }
  ...
  // Lyα BAO ghost ring (lv234+): visual ring offset from physical force ring.
  // Exclusive with baryonOscillations / oddRadioCircles / gravWaves.
  const lyaGhostRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 234 &&
    baryonOscillations.length === 0 &&
    oddRadioCircles.length === 0 &&
    gravWaves.length === 0 &&
    hazChance(lyaGhostRng, 0.35, 234, level)
  ) {
    const off = lyaGhostRng() < 0.5 ? LYAG_OFFSET : -LYAG_OFFSET;
    lyaGhosts.push({
      x: W * (0.30 + lyaGhostRng() * 0.40),
      y: topPad + playH * (0.30 + lyaGhostRng() * 0.40),
      rVis: LYAG_R,
      rPhys: LYAG_R + off,
    });
  }

  // Flexknot GWB hum (lv237+): twist frequency jumps at depth thresholds.
  // Exclusive with blueHum / hdHum / alens / gwb / gravEcho / gravWaves.
  const flexHumRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 237 &&
    !blueHumActive &&
    ...
    gravWaves.length === 0 &&
    !isoBireActive &&
    hazChance(flexHumRng, 0.35, 237, level)
  ) {
    flexHumActive = true;
  }

  // Magnetar birth chirp (lv242+): newborn magnetar whose twist chirps up each flare.
  // Exclusive with ordinary magnetars so the board has one clear flare language.
  const birthChirpRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 242 &&
    magnetars.length === 0 &&
    hazChance(birthChirpRng, 0.35, 242, level)
  ) {
    birthChirps.push({
      x: W * (0.28 + birthChirpRng() * 0.44),
      y: topPad + playH * (0.28 + birthChirpRng() * 0.40),
      period: Math.max(160, 280 - Math.max(0, (level - 242) * 6)),
      timer: 70 + Math.floor(birthChirpRng() * 80),
      releaseTimer: 0,
      chirpN: 0,
      twistSign: birthChirpRng() < 0.5 ? 1 : -1,
    });
  }

  // RM flare corridor (lv245+): diagonal LOS sweep; one twist per ball per pass.
  // Exclusive with FRB (global twist) and CME (horizontal plasma sweep).
  const rmFlareRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 245 &&
    frbSources.length === 0 &&
    !cme.active &&
    hazChance(rmFlareRng, 0.35, 245, level)
  ) {
    const travel = (rmFlareRng() < 0.5 ? 1 : -1) * (Math.PI * 0.25 + (rmFlareRng() - 0.5) * 0.4);
    rmFlares.push({
      tx: Math.cos(travel),
      ty: Math.sin(travel),
      pos: -1,
      period: Math.max(200, 320 - Math.max(0, (level - 245) * 5)),
      timer: 70 + Math.floor(rmFlareRng() * 90),
      twistSign: rmFlareRng() < 0.5 ? 1 : -1,
      hit: new WeakSet(),
    });
  }


  // Dimming DM shell (lv248+): expanding SNR-like shell with soft drag + weak refract.
  // Exclusive with ORC / gravWaves (same expanding-ring family).
  const dimShellRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 248 &&
    oddRadioCircles.length === 0 &&
    gravWaves.length === 0 &&
    hazChance(dimShellRng, 0.35, 248, level)
  ) {
    dimDmShells.push({
      x: W * (0.30 + dimShellRng() * 0.40),
      y: topPad + playH * (0.28 + dimShellRng() * 0.40),
      radius: DDS_R_MIN,
      phase: 'grow',
      timer: 0,
      litBins: new Array(DDS_LIT_BINS).fill(0),
    });
  }

  // Persistent radio nebula (lv251+): fixed soft halo — drag + tiny outward push.
  // Exclusive with silkDampingClouds (both soft-drag volumes).
  const prsNebRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 251 &&
    silkDampingClouds.length === 0 &&
    hazChance(prsNebRng, 0.35, 251, level)
  ) {
    prsNebulae.push({
      x: W * (0.28 + prsNebRng() * 0.44),
      y: topPad + playH * (0.28 + prsNebRng() * 0.40),
    });
  }

  // GWB-LSS anisotropic bias (lv254+): board-wide axis push with heading quadrupole weight.
  // Exclusive with HD/alens/gwb/flex/blue/isoBire hum-twist family.
  const gwbLssRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 254 &&
    !hdHumActive &&
    ...
    !uldmWaveActive &&
    gravWaves.length === 0 &&
    hazChance(gwbLssRng, 0.35, 254, level)
  ) {
    gwbLssBiases.push({
      theta0: gwbLssRng() * Math.PI * 2,
      accel: GWBLSS_ACCEL + Math.min(0.012, Math.max(0, (level - 254) * 0.0008)),
    });
  }

  // Ultralight DM undulation (lv257+): board-wide tiny speed-preserving twist wave.
  // Exclusive with flexHum / other global twist hums (same tell language).
  const uldmRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 257 &&
    !flexHumActive &&
    ...
    !blueHumActive &&
    !isoBireActive &&
    gwbLssBiases.length === 0 &&
    gravWaves.length === 0 &&
    hazChance(uldmRng, 0.35, 257, level)
  ) {
    uldmWaveActive = true;
  }

  // JWST SFE overproduction (lv262+): mild boost ellipse — overproduction, not damping.
  const sfeOverRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 262 &&
    silkDampingClouds.length === 0 &&
    prsNebulae.length === 0 &&
    hazChance(sfeOverRng, 0.35, 262, level)
  ) {
    sfeOvers.push({
      x: W * (0.28 + sfeOverRng() * 0.44),
      y: topPad + playH * (0.28 + sfeOverRng() * 0.40),
      rx: SFE_RX,
      ry: SFE_RY,
      axis: sfeOverRng() * Math.PI,
    });
  }

  // DE perturbation curtain (lv265+): OBB softens gravity + micro twist.
  const dePertRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 265 &&
    !quintomBreathActive &&
    ...
    ideSiphonBands.length === 0 &&
    signIdeSeams.length === 0 &&
    hazChance(dePertRng, 0.35, 265, level)
  ) {
    dePertCurtains.push({
      x: W * (0.30 + dePertRng() * 0.40),
      y: topPad + playH * (0.30 + dePertRng() * 0.40),
      angle: dePertRng() * Math.PI,
    });
  }

  // Effective -m_nu flow (lv268+): board-wide mild isotropic growth (toy of negative effective mass).
  const negNuRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 268 &&
    nuNullBands.length === 0 &&
    dualH0Seam === null &&
    hazChance(negNuRng, 0.35, 268, level)
  ) {
    negNuFlowActive = true;
  }

  // FRB old-population delayed burst (lv271+): long telegraph, milder twist.
  const frbDelayRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 271 &&
    frbSources.length === 0 &&
    rmFlares.length === 0 &&
    hazChance(frbDelayRng, 0.35, 271, level)
  ) {
    const period = Math.max(420, 560 - (level - 271) * 8);
    frbDelays.push({
      x: hazChance(frbDelayRng, 0.5) ? 4 : W - 4,
      y: topPad + playH * (0.2 + frbDelayRng() * 0.6),
      period,
      timer: period,
      fireAngle: 0,
      fired: false,
  ...
  // Spatial curvature keel (lv274+): edge band steers heading open(+)/closed(-).
  const curvKeelRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 274 &&
    bubbleUniverses.length === 0 &&
    cccBoundary === null &&
    hazChance(curvKeelRng, 0.35, 274, level)
  ) {
    curvKeels.push({ sign: hazChance(curvKeelRng, 0.5) ? 1 : -1 });
  }

  // Interacting HDE transfer belt (lv277+): one side axial push, other side mild drag.
  const ihdeRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 277 &&
    ...
    bigRip === null &&
    ideSiphonBands.length === 0 &&
    hazChance(ihdeRng, 0.35, 277, level)
  ) {
    ihdeBelts.push({
      y: topPad + playH * (0.28 + ihdeRng() * 0.44),
      halfW: IHDE_HALF,
      dir: hazChance(ihdeRng, 0.5) ? 1 : -1,
    });
  }

  // ─── Zone remix (gap levels) ───────────────────────────────────────────────
  // Memory-burdened PBH ember (lv282+): weak pull + periodic young-evaporation pulse.
  const mbeRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 282 &&
    microBHs.length === 0 &&
    ...
    hazChance(mbeRng, 0.35, 282, level)
  ) {
    memoryBurdenEmbers.push({
      x: W * (0.22 + mbeRng() * 0.56),
      y: topPad + playH * (0.22 + mbeRng() * 0.50),
      period: MBE_PERIOD,
  ...
  // ─── Zone remix (gap levels)
  // Hostless LRD seed (lv285+): Keplerian swirl, no host cocoon.
  const nakedSeedRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 285 &&
    littleRedDots.length === 0 &&
  ...
  const dressedPbhRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 288 &&
    frbMicrolenses.length === 0 &&
    ...
    primordialBHs.length === 0 &&
    gravitationalCaustics.length === 0 &&
    hazChance(dressedPbhRng, 0.35, 288, level)
  ) {
    dressedPbhs.push({
      x: W * (0.25 + dressedPbhRng() * 0.50),
  ...
  // F_AP anisotropy loom (lv291+): DESI shape diagnostic as velocity loom.
  const fapLoomRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 291 &&
    cosmicShears.length === 0 &&
    ...
    mBiasVeils.length === 0 &&
    !alensActive &&
    hazChance(fapLoomRng, 0.35, 291, level)
  ) {
    fapLooms.push({
      x: W * (0.28 + fapLoomRng() * 0.44),
      y: topPad + playH * (0.28 + fapLoomRng() * 0.40),
      rx: FAP_RX,
      ry: FAP_RY,
  ...
  // Resolvable PTA continuous wave (lv294+): directional sinusoid force.
  const ptaCwRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 294 &&
    !hdHumActive &&
  ...
  const scptWallRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 297 &&
    vacuums.length === 0 &&
    ...
    bubbleUniverses.length === 0 &&
    axionWalls.length === 0 &&
    hazChance(scptWallRng, 0.35, 297, level)
  ) {
    scptWalls.push({
      x: W * (0.28 + scptWallRng() * 0.44),
      y: topPad + playH * (0.28 + scptWallRng() * 0.40),
      r: SCPT_R0,
      respawnTimer: 0,
      passingBalls: new WeakSet(),
    });
  ...
  // Axion-string birefringence patchwork (lv302+).
  const abpRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 302 &&
    !isoBireActive &&
    cosmicBirefringences.length === 0 &&
    !ebParityActive &&
    hazChance(abpRng, 0.40, 302, level)
  ) {
    const n = 3 + Math.floor(abpRng() * 3);
    const edges: number[] = [];
    for (let i = 1; i < n; i++) edges.push(W * (i / n) + (abpRng() - 0.5) * W * 0.06);
    edges.sort((a, b) => a - b);
    const signs: (1 | -1)[] = [];
    let sgn: 1 | -1 = abpRng() < 0.5 ? 1 : -1;
    for (let i = 0; i < n; i++) { signs.push(sgn); sgn = sgn === 1 ? -1 : 1; }
    axionBirePatchwork = { edges, signs, lastStrip: new WeakMap() };
  }

  // Quadruple-image ghost lens (lv305+).
  const qglRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 305 &&
    dressedPbhs.length === 0 &&
    einsteinCrosses.length === 0 &&
    hazChance(qglRng, 0.40, 305, level)
  ) {
    quadGhostLenses.push({
      x: W * (0.28 + qglRng() * 0.44),
      y: topPad + playH * (0.28 + qglRng() * 0.40),
      flashTimer: 0,
      track: new WeakMap(),
    });
  }
  ...
  // EDE law blink (lv308+).
  const edeBlinkRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 308 &&
    !quintomBreathActive &&
    signIdeSeams.length === 0 &&
    hazChance(edeBlinkRng, 0.35, 308, level)
  ) {
    edeLawActive = true;
  }

  // Free-streaming cutoff blade (lv311+).
  const fsBladeRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 311 &&
    silkDampingClouds.length === 0 &&
    !cme.active &&
    hazChance(fsBladeRng, 0.40, 311, level)
  ) {
    const horizontal = fsBladeRng() < 0.5;
    const dir: 1 | -1 = fsBladeRng() < 0.5 ? 1 : -1;
    const pos = horizontal
      ? (dir > 0 ? launcherY + 40 : H - 20)
      : (dir > 0 ? 10 : W - 10);
    fsCutoffBlade = { horizontal, pos, dir, passingBalls: new WeakSet() };
  }

  // EB parity fault (lv314+).
  const ebParityRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 314 &&
    axionBirePatchwork === null &&
    !isoBireActive &&
    hazChance(ebParityRng, 0.40, 314, level)
  ) {
    ebParityActive = true;
  }

  // Measurement-disagreement dual field (lv317+).
  const measDualRng = makeRng((rng() * 0x100000000) >>> 0);
  if (
    anomalyKind === null &&
    level >= 317 &&
    cosmicShears.length === 0 &&
    fapLooms.length === 0 &&
    hazChance(measDualRng, 0.35, 317, level)
  ) {
    measDisagreeDuals.push({
      x: W * (0.28 + measDualRng() * 0.44),
      y: topPad + playH * (0.28 + measDualRng() * 0.40),
      rx: MEASDUAL_RX,
      ry: MEASDUAL_RY,
      axis: measDualRng() * Math.PI,
    });
  }
...
export function DotShotGame() {
  ...
  const G = useRef<GameState>({
    ...
    quadGhostLenses: [],
    edeLawActive: false,
    edeLawTimer: EDEBLINK_PERIOD,
    edeLawWarn: 0,
    edeLawBlink: 0,
    fsCutoffBlade: null,
    ebParityActive: false,
    measDisagreeDuals: [],
  ...
  const initLevel = useCallback((lv: number) => {
    ...
    g.gwBackgroundAntiPhase = gwBackgroundAntiPhase;
    ...
    g.s8Seams = s8Seams;
    g.barySofts = barySofts;
    g.chameleons = chameleons;
    g.isoBireActive = isoBireActive;
    g.isoBireBeta = isoBireBeta;
    g.lyaGhosts = lyaGhosts;
    g.flexHumActive = flexHumActive;
    g.uldmWaveActive = uldmWaveActive;
    g.birthChirps = birthChirps;
    g.rmFlares = rmFlares;
    g.dimDmShells = dimDmShells;
    g.prsNebulae = prsNebulae;
    g.gwbLssBiases = gwbLssBiases;
    g.sfeOvers = sfeOvers;
    g.dePertCurtains = dePertCurtains;
    g.negNuFlowActive = negNuFlowActive;
    g.frbDelays = frbDelays;
    g.curvKeels = curvKeels;
    g.ihdeBelts = ihdeBelts;
    g.memoryBurdenEmbers = memoryBurdenEmbers;
    g.nakedLrdSeeds = nakedLrdSeeds;
    g.dressedPbhs = dressedPbhs;
    g.fapLooms = fapLooms;
    g.ptaCw = ptaCw;
    g.scptWalls = scptWalls;
    g.axionBirePatchwork = axionBirePatchwork;
    g.quadGhostLenses = quadGhostLenses;
    g.edeLawActive = edeLawActive;
    g.edeLawTimer = EDEBLINK_PERIOD;
    g.edeLawWarn = 0;
    g.edeLawBlink = 0;
    g.fsCutoffBlade = fsCutoffBlade;
    g.ebParityActive = ebParityActive;
    g.measDisagreeDuals = measDisagreeDuals;
    ...
    // Paper mood: wordless cream-board grain (deterministic per level; layout rng untouched).
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      const pm = g.paperMood ?? emptyPaperMood();
      if (!g.paperMood) g.paperMood = pm;
      const moodId = pm.id;
      const moodRare = pm.rare;
      // Advance rare timed overlays (anti-snow echo / total still).
      if (pm.rareTimer > 0 && g.phase !== 'paused' && g.phase !== 'idle') {
        pm.rareTimer--;
      }
      const rareAntiSnow = moodRare === 'antiSnowEcho' && pm.rareTimer > 0;
      const rareStill = moodRare === 'totalStill' && pm.rareTimer > 0;
      ...
      if (g.phase !== 'idle') {
        ...
        const cadenceMul = moodId === 'wrongCadence' ? (0.55 + pm.param * 1.1) : 1;
        if (g.bgClusterTimer <= 0 && g.bgDots.length < bgCap) {
          g.bgClusterTimer = Math.floor((55 + Math.floor(Math.random() * 70) + Math.floor(depthFactor(g.level) * 40)) * cadenceMul);
          let edge = depthEdgeBias(g.level);
          if (moodId === 'rimAsh') edge = Math.min(1, edge + 0.35);
          let cx = 60 + Math.random() * (W - 120);
          let cy = 60 + Math.random() * (H - 120);
          if (moodId === 'asymmetricAsh' && Math.random() < 0.55) {
            cx = pm.ashSide < 0
        ...
        // quietRearrange: also rare micro-rearrange while aiming (very slow).
        const unobsAim = moodId === 'quietRearrange' && g.phase === 'aiming' && (g.frame % 97 === 0);
        if ((g.level >= 75 && g.phase === 'firing') || unobsAim) {
          g.unobservedTimer--;
          if (g.unobservedTimer <= 0 || unobsAim) {
            g.unobservedTimer = unobsAim ? 90 : 30 + Math.floor(Math.random() * 30);
            ...
            if (anchor) {
              ...
              const reformN = Math.min(moved, unobsAim ? 4 : 10, room);
      ...
      const bg = g.bgDots;
      let hollowR = depthHollow(g.level) * Math.min(W, H) * 0.28;
      if (moodId === 'centerAbsence') hollowR *= 1.35;
      if (rareInvHollow) hollowR *= 0.35; // inverted: less center hollow; edge skip below
      ...
      let dustStill = g.anomalyKind === 'silence' ? 0.1 : 1;
      if (rareStill) dustStill *= 0.08;
      ...
      for (let bi = 0; bi < bg.length; bi++) {
        ...
        // invertedHollow rare: skip near edges instead of center
        if (!skipBg && edgeSkipR2 > 0) {
          const hdx = d.x - halfW, hdy = d.y - halfH;
        ...
        if (!skipBg) {
          ...
          // dualDrift: faint 1px phase-lag ghost
          if (moodId === 'dualDrift') {
            ctx.globalAlpha = a * 0.22;
            ctx.fillRect(Math.round(d.x + drawDx - 1), Math.round(d.y + drawDy), 1, 1);
          }
        }
        }
      ...
      if (g.gwBackgroundActive) {
        ...
        for (let ci = 0; ci < gwbCorners.length; ci++) {
          const [gcx, gcy, gsx, gsy] = gwbCorners[ci];
          // Anti-phase rare: TL/BR vs TR/BL blink out of phase (diagonal pairs).
          const phaseFlip = g.gwBackgroundAntiPhase && (ci === 1 || ci === 2) ? Math.PI : 0;
          const cornerPulse = 0.55 + 0.45 * Math.abs(Math.sin(g.frame * 0.06 + phaseFlip));
          for (let k = 0; k < 4; k++) {
            const a = (k / 3) * (Math.PI / 2);
            const rr = 6 + 2 * Math.sin(g.frame * 0.06 + k + phaseFlip);
            ctx.globalAlpha = cornerPulse * (0.55 + 0.45 * (k % 2));
      ...
      // ── Lyα BAO ghost ring: rust-white solid ring + offset ghost dots ──
      for (const lg of g.lyaGhosts) {
        for (let i = 0; i < 36; i++) {
          if ((i + ((g.frame * 0.0015) | 0)) % 4 === 0) continue;
          const a = (i / 36) * Math.PI * 2;
          const px = lg.x + Math.cos(a) * lg.rVis;
          const py = lg.y + Math.sin(a) * lg.rVis;
          if (px >= 4 && px <= W - 4 && py >= 4 && py <= H - 4) {
            ctx.fillStyle = '#807870';
            ctx.globalAlpha = 0.10 + 0.04 * Math.abs(Math.sin(i * 0.3 + g.frame * 0.0015));
            ctx.fillRect(Math.round(px), Math.round(py), 2, 1);
          }
          const gx = lg.x + Math.cos(a) * lg.rPhys;
          const gy = lg.y + Math.sin(a) * lg.rPhys;
          if (gx >= 4 && gx <= W - 4 && gy >= 4 && gy <= H - 4) {
            ctx.fillStyle = '#807870';
            ctx.globalAlpha = 0.06;
      ...
      // ── Interacting HDE belts: two-tone missing horizontal band ──
      for (const ib of g.ihdeBelts) {
        for (let i = 0; i < 32; i++) {
          if (i % 5 === 2) continue;
          const t = i / 31;
          const x = W * (0.08 + t * 0.84);
          const yLo = ib.y - ib.halfW * 0.55;
          const yHi = ib.y + ib.halfW * 0.55;
          ctx.globalAlpha = 0.16 + 0.08 * Math.abs(Math.sin(g.frame * 0.016 + i));
          ctx.fillStyle = '#687888';
          ctx.fillRect(Math.round(x) - 1, Math.round(yLo) - 1, 2, 2);
          ctx.fillStyle = '#905858';
          ctx.fillRect(Math.round(x + ib.dir * 2) - 1, Math.round(yHi) - 1, 2, 2);
        }
        ctx.globalAlpha = 1;
      }

      // ── Spatial curvature keels: breathing missing arcs along the rim ──
      for (const ck of g.curvKeels) {
        const col = ck.sign > 0 ? '#687888' : '#905858';
        ctx.fillStyle = col;
        const breath = 0.5 + 0.5 * Math.sin(g.frame * 0.018);
        for (let i = 0; i < 36; i++) {
          if (i % 5 === 2) continue;
          const t = i / 36;
          const a = t * Math.PI * 2 + g.frame * 0.002;
          const r = Math.min(W, H) * 0.48 - CURVK_BAND * 0.35 * breath;
          const x = W * 0.5 + Math.cos(a) * r;
          const y = H * 0.5 + Math.sin(a) * r;
          // only near rim (clip interior)
          if (x > CURVK_BAND * 1.2 && x < W - CURVK_BAND * 1.2 && y > CURVK_BAND * 1.2 && y < H - CURVK_BAND * 1.2) continue;
      ...
      // ── Effective -m_nu flow: corners gently inflate (growth tell) ──
      if (g.negNuFlowActive) {
        const inflate = 1 + 0.35 * Math.sin(g.frame * 0.012);
        ctx.fillStyle = '#8a7060';
        const corners: [number, number][] = [
          [10, 10], [W - 10, 10], [10, H - 10], [W - 10, H - 10],
        ];
        for (const [cx, cy] of corners) {
          ctx.globalAlpha = 0.10 + 0.08 * Math.abs(Math.sin(g.frame * 0.02 + cx));
          ctx.fillRect(Math.round(cx - inflate), Math.round(cy - inflate), 2, 2);
          ctx.fillRect(Math.round(cx + inflate), Math.round(cy), 1, 1);
        }
        ctx.globalAlpha = 1;
      }

      // ── Ultralight DM undulation: background dust sways in phase (minimal body) ──
      if (g.uldmWaveActive) {
        const sway = Math.sin(g.frame * ULDM_OMEGA) * 2.2;
        ctx.fillStyle = '#6890a8';
        for (let i = 0; i < 18; i++) {
          const bx = 12 + (i * 37) % (W - 24);
          const by = 20 + (i * 53) % (H - 40);
          ctx.globalAlpha = 0.08 + 0.06 * Math.abs(Math.sin(g.frame * ULDM_OMEGA + i));
          ctx.fillRect(Math.round(bx + sway), Math.round(by), 1, 1);
          ctx.fillRect(Math.round(bx + sway * 0.5), Math.round(by + 3), 1, 1);
        }
        ctx.globalAlpha = 1;
      }

      // ── Flexknot GWB hum: faint horizontal knot lines + depth-stepped corner blink ──
      if (g.flexHumActive) {
        ctx.fillStyle = '#586870';
        for (const yf of [1 / 3, 2 / 3]) {
          const y = H * yf;
          for (let x = 8; x < W - 8; x += 9) {
            if (((x / 9) + ((g.frame * 0.0015) | 0)) % 3 === 0) continue;
            ctx.globalAlpha = 0.06 + 0.04 * Math.abs(Math.sin(x * 0.04 + g.frame * 0.0015));
            ctx.fillRect(x, Math.round(y), 2, 1);
          }
        }
        const fhMargin = 11;
        const fhCorners: [number, number][] = [
          [fhMargin, fhMargin], [W - fhMargin, fhMargin],
          [fhMargin, H - fhMargin], [W - fhMargin, H - fhMargin],
        ];
        for (const [bx, by] of fhCorners) {
          let omega = FLEXHUM_W0;
          if (by / H >= 2 / 3) omega = FLEXHUM_W2;
          else if (by / H >= 1 / 3) omega = FLEXHUM_W1;
          const pulse = 0.30 + 0.70 * Math.abs(Math.sin(g.frame * omega + bx * 0.01));
          ctx.globalAlpha = 0.07 + 0.09 * pulse;
          ctx.fillRect(Math.round(bx) - 1, Math.round(by) - 1, 2, 2);
        }
      ...
      // ── Baryonic feedback softening: copper-ash outward sparse streaks ──
      for (const bs of g.barySofts) {
        const ca = Math.cos(bs.axis), sa = Math.sin(bs.axis);
        ctx.fillStyle = '#786858';
        for (let i = 0; i < 14; i++) {
          if ((i + ((g.frame * 0.0015) | 0)) % 3 === 0) continue;
          const ang = (i / 14) * Math.PI * 2;
          const rr = 0.35 + (i % 4) * 0.15;
          const lx = Math.cos(ang) * bs.rx * rr;
          const ly = Math.sin(ang) * bs.ry * rr;
          const px = bs.x + ca * lx - sa * ly;
          const py = bs.y + sa * lx + ca * ly;
          if (px < 4 || px > W - 4 || py < 4 || py > H - 4) continue;
          ctx.globalAlpha = 0.07 + 0.05 * Math.abs(Math.sin(i + g.frame * 0.0015));
          ctx.fillRect(Math.round(px), Math.round(py), 2, 1);
        }
        ctx.globalAlpha = 1;
      }

      // ── Chameleon screening: moss-gray broken arcs that dim near pegs ──
      for (const ch of g.chameleons) {
        let n = 0;
        const nR2 = CHAM_NEIGHBOR * CHAM_NEIGHBOR;
        for (const peg of g.pegs) {
          if (peg.cleared || peg.mudBroken) continue;
          const pdx = peg.x - ch.x, pdy = peg.y - ch.y;
          if (pdx * pdx + pdy * pdy < nR2) n++;
        }
        const screen = Math.min(1, n * 0.18);
        if (screen > 0.85) { ctx.globalAlpha = 1; continue; }
        ctx.fillStyle = '#687868';
        for (let i = 0; i < 20; i++) {
          if ((i + ((g.frame * 0.0015) | 0)) % 3 === 0) continue;
          const a = (i / 20) * Math.PI * 2;
          const rr = CHAM_RANGE * 0.72;
          const px = ch.x + Math.cos(a) * rr;
          const py = ch.y + Math.sin(a) * rr;
          if (px < 4 || px > W - 4 || py < 4 || py > H - 4) continue;
          ctx.globalAlpha = (0.10 - screen * 0.08) * (0.5 + 0.5 * Math.abs(Math.sin(i + g.frame * 0.0015)));
          ctx.fillRect(Math.round(px), Math.round(py), 2, 1);
        }
        ctx.globalAlpha = 1;
      }
      ...
      for (const hv of g.hyperStars) {
        ...
        if (hv.x < -60 || hv.x > W + 60) {
          if (hv.returns && !hv.returned) {
            hv.returned = true;
            hv.respawnTimer = 36;
            hv.warnFromLeft = !hv.warnFromLeft;
            hv.warnY += 50;
          } else {
            hv.returned = false;
      ...
      for (const cls of g.collisionlessShocks) {
        ...
        const armBase = cls.inverted ? hang : hang + Math.PI;
        const arms = [armBase - cls.armSpread, armBase + cls.armSpread];
      ...
      for (const bc of g.bulletClusters) {
        ...
        const dmX   = bc.x + bcDir * BC_DM_LAG * (bc.dmContra ? -1 : 1);
      ...
      if (g.cmeActive) {
        ...
        const tiltTan = Math.tan(g.cmeTilt);
        if (g.cmeY < 0) {
          g.cmeTimer--;
          if (g.cmeTimer <= WARN && g.cmeTimer > 0) {
            ...
            // Warn bar follows the same tilt as the coming front.
            for (let bx = 0; bx < W; bx += 4) {
              const wy = launcherY + 30 + (bx - W * 0.5) * tiltTan;
              ctx.fillRect(bx, Math.round(wy), 4, 4);
            }
            ctx.globalAlpha = 1;
          }
          ...
          for (let bx = 0; bx < W; bx += 3) {
            ...
            const frontY = g.cmeY + (bx - W * 0.5) * tiltTan + ripple;
            ctx.globalAlpha = 0.7 + Math.abs(Math.sin(bx * 0.05 + g.frame * 0.3)) * 0.3;
            ctx.fillRect(bx, Math.round(frontY), 2, 2);
          }
          for (let i = 0; i < 70; i++) {
            const bx = Math.random() * W;
            const depth = Math.random() * Math.random(); // biased toward 0 = just behind the front
            const frontY = g.cmeY + (bx - W * 0.5) * tiltTan;
            const by = frontY - 4 - depth * BAND;
          ...
          ctx.globalAlpha = 1;
          if (g.cmeY > H + Math.abs(tiltTan) * W * 0.5) { g.cmeY = -1; g.cmeTimer = g.cmePeriod; }
        }
      }
      ...
      if (g.reionActive) {
        if (g.reionY < 0) {
          ...
          const reionTiltTan = Math.tan(g.reionTilt);
          // Leading purple front dots scrolling left→right (oblique when tilted)
          ctx.fillStyle = '#7b5cff';
          const phase = g.frame * 0.8;
          for (let bx = ((phase % 8) - 8); bx < W; bx += 8) {
            const frontY = g.reionY + (bx - W * 0.5) * reionTiltTan;
            ctx.globalAlpha = 0.75 + 0.2 * Math.sin(bx * 0.08 + g.frame * 0.05);
            ctx.fillRect(Math.round(bx), Math.round(frontY) - 1, 2, 2);
          }
          ...
          for (let row = 1; row <= 3; row++) {
            ...
            for (let bx = 0; bx < W; bx += 10 + row) {
              const wakeY = g.reionY + (bx - W * 0.5) * reionTiltTan - row * 5;
              ctx.fillRect(bx, Math.round(wakeY), 1, 1);
            }
          }
          ctx.globalAlpha = 1;
          if (g.reionY > H + Math.abs(reionTiltTan) * W * 0.5) { g.reionY = -1; g.reionTimer = g.reionPeriod; }
        }
      }
      ...
      // ── Magnetar birth chirps: newborn flare whose twist chirps up each pulse ──
      for (const bc of g.birthChirps) {
        if (bc.releaseTimer > 0) {
          bc.releaseTimer--;
          if (bc.releaseTimer <= 0) bc.chirpN++; // climb after each pulse so first uses TWIST0
        } else {
          bc.timer--;
          if (bc.timer <= 0) {
            bc.releaseTimer = MBC_RELEASE;
            bc.timer = bc.period;
            spawnBurst(g, bc.x, bc.y, 10, 10, '#ffe8a0');
          }
        }
        const charging = bc.releaseTimer <= 0 && bc.timer <= MBC_WARN;
        const dens = Math.min(4, 1 + bc.chirpN); // rings densify as chirp climbs
        const corePulse = 0.4 + Math.abs(Math.sin(g.frame * (charging ? 0.28 : 0.06))) * 0.6;
        // Cooler field arches than ordinary magnetars - ice-amber newborn motif
        ctx.fillStyle = charging ? '#fff0c0' : '#d8b860';
        for (let ring = 0; ring < dens; ring++) {
          const rr = 12 + ring * 8;
          const n = 10 + ring * 4 + Math.min(8, bc.chirpN * 2);
          for (let i = 0; i < n; i++) {
            const a = (i / n) * Math.PI * 2 + g.frame * 0.012 * (ring % 2 === 0 ? 1 : -1) * bc.twistSign;
            ctx.globalAlpha = corePulse * (0.4 + (i % 2) * 0.3) * (charging ? 1 : 0.65);
            ctx.fillRect(Math.round(bc.x + Math.cos(a) * rr) - 1, Math.round(bc.y + Math.sin(a) * rr) - 1, 2, 2);
          }
        }
        if (bc.releaseTimer > 0) {
          const rt = 1 - bc.releaseTimer / MBC_RELEASE;
          ctx.fillStyle = '#fff6d0';
          for (let i = 0; i < 48; i++) {
            const a = (i / 48) * Math.PI * 2;
            const rr = rt * MBC_RANGE;
            ctx.globalAlpha = (1 - rt) * 0.85;
            ctx.fillRect(Math.round(bc.x + Math.cos(a) * rr) - 1, Math.round(bc.y + Math.sin(a) * rr) - 1, 2, 2);
          }
          // chirp-twist tell: a short spiral arm during release
          ctx.fillStyle = '#a8d8ff';
          for (let i = 0; i < 18; i++) {
            const t = i / 18;
            const a = t * Math.PI * 2.4 * bc.twistSign + g.frame * 0.04;
            const rr = 8 + t * 36;
            ctx.globalAlpha = (1 - rt) * 0.7;
            ctx.fillRect(Math.round(bc.x + Math.cos(a) * rr) - 1, Math.round(bc.y + Math.sin(a) * rr) - 1, 2, 2);
          }
        ...
        ctx.globalAlpha = 1;
        drawSolidCircle(ctx, bc.x, bc.y, 6, '#2c2e38');
        ctx.fillStyle = (charging || corePulse > 0.85) ? '#ffffff' : '#ffe8a0';
        ctx.globalAlpha = corePulse;
        ctx.fillRect(Math.round(bc.x) - 2, Math.round(bc.y) - 2, 4, 4);
        ctx.globalAlpha = 1;
      }

      // ── RM flare corridors: diagonal LOS band sweep (update + draw) ──
      for (const rf of g.rmFlares) {
        const span = Math.hypot(W, H) + 120;
        if (rf.pos < 0) {
          rf.timer--;
          if (rf.timer <= RMF_WARN && rf.timer > 0) {
            const wt = 1 - rf.timer / RMF_WARN;
            ctx.fillStyle = '#3a9a78';
            // telegraph: faint dots along the upcoming corridor midline
            for (let t = 0; t < span; t += 14) {
              const x = rf.tx * (t - 40);
              const y = rf.ty * (t - 40);
              if (x < -4 || x > W + 4 || y < -4 || y > H + 4) continue;
              ctx.globalAlpha = 0.12 * wt * (0.5 + 0.5 * Math.abs(Math.sin(t * 0.05 + g.frame * 0.2)));
              ctx.fillRect(Math.round(x) - 1, Math.round(y) - 1, 2, 2);
            }
            ctx.globalAlpha = 1;
          }
          ...
          rf.pos += RMF_SPD;
          // body of the corridor (greenish RM plasma)
          ctx.fillStyle = '#3a9a78';
          for (let t = -span; t < span; t += 5) {
            const cx = rf.tx * rf.pos + -rf.ty * t;
            const cy = rf.ty * rf.pos + rf.tx * t;
            if (cx < -6 || cx > W + 6 || cy < -6 || cy > H + 6) continue;
            const edge = Math.abs(t) / (span * 0.55);
            if (edge > 1) continue;
            ctx.globalAlpha = (0.35 + 0.25 * Math.abs(Math.sin(t * 0.08 + g.frame * 0.15))) * (1 - edge * 0.5);
            ctx.fillRect(Math.round(cx) - 1, Math.round(cy) - 1, 2, 2);
          }
          // leading edge blink
          ctx.fillStyle = '#8fe0c0';
          for (let t = -span * 0.4; t < span * 0.4; t += 4) {
            const cx = rf.tx * rf.pos + -rf.ty * t;
            const cy = rf.ty * rf.pos + rf.tx * t;
            if (cx < -4 || cx > W + 4 || cy < -4 || cy > H + 4) continue;
            ctx.globalAlpha = 0.55 + 0.45 * Math.abs(Math.sin(g.frame * 0.35 + t * 0.04));
            ctx.fillRect(Math.round(cx) - 1, Math.round(cy) - 1, 2, 2);
          }
          ctx.globalAlpha = 1;
          if (rf.pos > span) {
      ...
      // ── Dimming DM shells: expanding grey-green SNR shell (update + draw) ──
      for (const ds of g.dimDmShells) {
        for (let bi = 0; bi < ds.litBins.length; bi++) if (ds.litBins[bi] > 0) ds.litBins[bi]--;
        if (ds.phase === 'grow') {
          ds.radius += (DDS_R_MAX - DDS_R_MIN) / DDS_GROW_FRAMES;
          if (ds.radius >= DDS_R_MAX) { ds.radius = DDS_R_MAX; ds.phase = 'fadeOut'; ds.timer = DDS_FADE_DUR; }
        } else if (ds.phase === 'fadeOut') {
          ds.timer--;
          if (ds.timer <= 0) { ds.phase = 'recondense'; ds.timer = DDS_RECONDENSE_DUR; }
        } else {
          ds.timer--;
          if (ds.timer <= 0) { ds.phase = 'grow'; ds.radius = DDS_R_MIN; }
        }
        const baseAlpha = ds.phase === 'grow' ? Math.min(0.50, 0.28 + DDS_BAND_HALF / ds.radius)
                        : ds.phase === 'fadeOut' ? Math.min(0.50, 0.28 + DDS_BAND_HALF / ds.radius) * (ds.timer / DDS_FADE_DUR)
                        : 0;
        if (ds.phase !== 'recondense' && baseAlpha > 0.002) {
          ctx.fillStyle = '#687070';
          const nDots = Math.max(28, Math.round((2 * Math.PI * ds.radius) / 7));
          for (let i = 0; i < nDots; i++) {
            const a = (i / nDots) * Math.PI * 2;
            ctx.globalAlpha = baseAlpha * (0.55 + (i % 2) * 0.35);
            ctx.fillRect(Math.round(ds.x + Math.cos(a) * ds.radius) - 1, Math.round(ds.y + Math.sin(a) * ds.radius) - 1, 2, 2);
          }
          ctx.globalAlpha = 1;
        }
        const binWidth = (Math.PI * 2) / DDS_LIT_BINS;
        for (let bi = 0; bi < ds.litBins.length; bi++) {
          if (ds.litBins[bi] <= 0) continue;
          const lt = ds.litBins[bi] / DDS_LIT_DUR;
          const binCenter = bi * binWidth;
          ctx.fillStyle = '#98a8a0';
          for (let k = -3; k <= 3; k++) {
            const a = binCenter + (k / 3) * (binWidth / 2);
            ctx.globalAlpha = lt * 0.75;
            ctx.fillRect(Math.round(ds.x + Math.cos(a) * ds.radius) - 1, Math.round(ds.y + Math.sin(a) * ds.radius) - 1, 2, 2);
          }
          ctx.globalAlpha = 1;
        }
        if (ds.phase === 'recondense') {
          const rt = 1 - ds.timer / DDS_RECONDENSE_DUR;
          const rr = DDS_R_MAX * (1 - rt);
          ctx.fillStyle = '#687070';
          for (let i = 0; i < 18; i++) {
            const a = (i / 18) * Math.PI * 2;
            ctx.globalAlpha = 0.22 * rt;
      ...
      // ── Persistent radio nebulae: fixed radio-green halo + slow core blink ──
      for (const prs of g.prsNebulae) {
        const pulse = 0.35 + 0.25 * (0.5 + 0.5 * Math.sin(g.frame * 0.04));
        ctx.fillStyle = '#3a9a78';
        const nDots = 40;
        for (let i = 0; i < nDots; i++) {
          const a = (i / nDots) * Math.PI * 2 + g.frame * 0.003;
          const rr = PRS_R * (0.55 + 0.45 * ((i * 7) % 10) / 10);
          ctx.globalAlpha = pulse * (0.25 + (i % 3) * 0.12);
          ctx.fillRect(Math.round(prs.x + Math.cos(a) * rr) - 1, Math.round(prs.y + Math.sin(a) * rr) - 1, 2, 2);
        }
        // sparse outer ghost ring
        for (let i = 0; i < 24; i++) {
          const a = (i / 24) * Math.PI * 2;
          ctx.globalAlpha = 0.18 * pulse;
          ctx.fillRect(Math.round(prs.x + Math.cos(a) * PRS_R) - 1, Math.round(prs.y + Math.sin(a) * PRS_R) - 1, 1, 1);
        }
        ctx.globalAlpha = 1;
        drawSolidCircle(ctx, prs.x, prs.y, 5, '#1a4038');
        ctx.fillStyle = '#8fe0c0';
        ctx.globalAlpha = 0.4 + 0.5 * Math.abs(Math.sin(g.frame * 0.05));
        ctx.fillRect(Math.round(prs.x) - 1, Math.round(prs.y) - 1, 3, 3);
        ctx.globalAlpha = 1;
      }

      // ── GWB-LSS anisotropic bias: faint axis ticks + sparse structure drift ──
      for (const gb of g.gwbLssBiases) {
        const ax = Math.cos(gb.theta0), ay = Math.sin(gb.theta0);
        ctx.fillStyle = '#6a5890';
        // four soft axis markers near the corners, projected onto the preferred axis
        const corners = [[18, 18], [W - 18, 18], [18, H - 18], [W - 18, H - 18]];
        for (const [cx, cy] of corners) {
          const along = cx * ax + cy * ay;
          const px = ax * along * 0.02 + cx * 0.98;
          const py = ay * along * 0.02 + cy * 0.98;
          ctx.globalAlpha = 0.22 + 0.12 * Math.abs(Math.sin(g.frame * 0.02 + cx * 0.01));
          ctx.fillRect(Math.round(px) - 1, Math.round(py) - 1, 2, 2);
          ctx.fillRect(Math.round(px + ax * 5) - 1, Math.round(py + ay * 5) - 1, 1, 1);
        }
        // sparse LSS dust drifting along the axis
        for (let i = 0; i < 14; i++) {
          const t = ((g.frame * 0.35 + i * 47) % (W + H)) - 40;
          const x = ax * t + -ay * ((i * 37) % 80 - 40) + W * 0.5 * (1 - Math.abs(ax));
          const y = ay * t + ax * ((i * 53) % 80 - 40) + H * 0.35;
          if (x < 4 || x > W - 4 || y < 4 || y > H - 4) continue;
      ...
      // ── JWST SFE overproduction: bone-white outward speckles ──
      for (const sf of g.sfeOvers) {
        const ca = Math.cos(sf.axis), sa = Math.sin(sf.axis);
        ctx.fillStyle = '#c8c0b0';
        for (let i = 0; i < 36; i++) {
          const u = ((i * 17) % 20) / 10 - 1;
          const v = ((i * 29) % 20) / 10 - 1;
          if (u * u + v * v > 1) continue;
          const wx = sf.x + ca * u * sf.rx - sa * v * sf.ry;
          const wy = sf.y + sa * u * sf.rx + ca * v * sf.ry;
          const outward = 1 + 0.08 * Math.sin(g.frame * 0.02 + i);
          ctx.globalAlpha = 0.18 + 0.12 * Math.abs(Math.sin(g.frame * 0.015 + i));
          ctx.fillRect(Math.round(sf.x + (wx - sf.x) * outward) - 1, Math.round(sf.y + (wy - sf.y) * outward) - 1, 2, 2);
        }
        ctx.globalAlpha = 1;
      }

      // ── DE perturbation curtains: frost-blue missing membrane ──
      for (const dc of g.dePertCurtains) {
        const ca = Math.cos(dc.angle), sa = Math.sin(dc.angle);
        ctx.fillStyle = '#6888a0';
        for (let i = 0; i < 28; i++) {
          if (i % 5 === 2) continue; // missing dots = "perturbed"
          const t = (i / 27) - 0.5;
          const lx = t * DEPERT_LEN;
          const ly = Math.sin(g.frame * 0.02 + i) * (DEPERT_THICK * 0.35);
          const wx = dc.x + ca * lx - sa * ly;
          const wy = dc.y + sa * lx + ca * ly;
          ctx.globalAlpha = 0.20 + 0.10 * Math.abs(Math.sin(g.frame * 0.015 + i));
          ctx.fillRect(Math.round(wx) - 1, Math.round(wy) - 1, 2, 2);
        }
        ctx.globalAlpha = 1;
      }
      ...
      // ── Memory-burdened PBH embers (lv282+) ──
      for (const mbe of g.memoryBurdenEmbers) {
        if (mbe.releaseTimer > 0) {
          mbe.releaseTimer--;
        } else {
          mbe.timer--;
          if (mbe.timer <= 0) {
            mbe.releaseTimer = MBE_RELEASE;
            mbe.timer = mbe.period;
          }
        }
        ...
        if (mbe.releaseTimer > 0) {
          ...
          for (let i = 0; i < 36; i++) {
            if (i % 3 === 0) continue;
            const a = (i / 36) * Math.PI * 2;
            const rr = 8 + rt * (MBE_PULSE_RANGE - 8);
            ctx.globalAlpha = (1 - rt) * 0.7;
            ctx.fillRect(
              Math.round(mbe.x + Math.cos(a) * rr),
              Math.round(mbe.y + Math.sin(a) * rr),
              2, 2,
            );
          }
        }
      ...
      // ── Hostless LRD seeds (lv285+) ──
      for (const ns of g.nakedLrdSeeds) {
        ctx.fillStyle = '#a03028';
        ctx.globalAlpha = 0.85;
        ctx.fillRect(Math.round(ns.x) - 2, Math.round(ns.y) - 2, 4, 4);
        ctx.fillStyle = '#6a98a8';
        for (let i = 0; i < 12; i++) {
          if (i % 2 === 0) continue;
          const a = (i / 12) * Math.PI * 2 + g.frame * 0.02;
          const rr = 18 + (i % 3) * 6;
          ctx.globalAlpha = 0.25;
          ctx.fillRect(Math.round(ns.x + Math.cos(a) * rr), Math.round(ns.y + Math.sin(a) * rr), 1, 1);
        }
        ctx.globalAlpha = 1;
      }
      ...
      for (const dp of g.dressedPbhs) {
        ...
        if (dp.flashTimer > 0) {
          ...
          ctx.fillStyle = '#c8b090';
          for (const off of [-0.08, 0.08] as const) {
            for (let i = 0; i < 20; i++) {
              const a = (i / 20) * Math.PI * 2;
              const rr = DPBH_RING_R * (1 + off);
              ctx.globalAlpha = life * 0.5;
              ctx.fillRect(Math.round(dp.x + Math.cos(a) * rr), Math.round(dp.y + Math.sin(a) * rr), 2, 1);
            }
          }
        }
        ctx.globalAlpha = 1;
      ...
      // ── Supercooled phase-transition walls (lv297+) ──
      for (const sw of g.scptWalls) {
        if (sw.respawnTimer > 0) {
          sw.respawnTimer--;
          if (sw.respawnTimer <= 0) { sw.r = SCPT_R0; sw.passingBalls = new WeakSet(); }
          continue;
        }
        sw.r += SCPT_GROW;
        if (sw.r >= SCPT_RMAX) { sw.respawnTimer = SCPT_RESPAWN; sw.r = 0; continue; }
        ctx.fillStyle = '#2a2824';
        for (let i = 0; i < 48; i++) {
          if (i % 3 === 0) continue;
          const a = (i / 48) * Math.PI * 2;
          ctx.globalAlpha = 0.45;
          ctx.fillRect(Math.round(sw.x + Math.cos(a) * sw.r), Math.round(sw.y + Math.sin(a) * sw.r), 2, 2);
        }
        ctx.fillStyle = '#d8c8a0';
        for (let i = 0; i < 8; i++) {
          const a = (i / 8) * Math.PI * 2 + g.frame * 0.01;
          const rr = sw.r * (0.7 + 0.1 * (i % 3));
          ctx.globalAlpha = 0.2;
          ctx.fillRect(Math.round(sw.x + Math.cos(a) * rr), Math.round(sw.y + Math.sin(a) * rr), 1, 1);
        }
        ctx.globalAlpha = 1;
      }

      // ── Measurement-disagreement dual field (lv317+) ──
      for (const md of g.measDisagreeDuals) {
        const phase = Math.sin(g.frame * MEASDUAL_OMEGA);
        const c = Math.cos(md.axis), sn = Math.sin(md.axis);
        for (let i = 0; i < 36; i++) {
          const a = (i / 36) * Math.PI * 2;
          const px = md.rx * Math.cos(a);
          const py = md.ry * Math.sin(a);
          const wx = md.x + c * px - sn * py;
      ...
      // ── Free-streaming cutoff blade (lv311+) ──
      if (g.fsCutoffBlade) {
        const bl = g.fsCutoffBlade;
        bl.pos += bl.dir * FSBLADE_SPD;
        const done = bl.horizontal
          ? (bl.dir > 0 ? bl.pos > H + 20 : bl.pos < launcherY)
          : (bl.dir > 0 ? bl.pos > W + 20 : bl.pos < -20);
        if (done) {
          bl.dir = (bl.dir === 1 ? -1 : 1);
          bl.pos = bl.horizontal
            ? (bl.dir > 0 ? launcherY + 40 : H - 20)
            : (bl.dir > 0 ? 10 : W - 10);
          bl.passingBalls = new WeakSet();
        }
        ...
        if (bl.horizontal) {
          for (let x = 4; x < W - 4; x += 5) {
            if (x % 15 === 0) continue;
            ctx.globalAlpha = 0.35;
            ctx.fillRect(x, Math.round(bl.pos), 2, 1);
          }
        } else {
          for (let y = 4; y < H - 4; y += 5) {
            if (y % 15 === 0) continue;
            ctx.globalAlpha = 0.35;
            ctx.fillRect(Math.round(bl.pos), y, 1, 2);
          }
        }
        ctx.globalAlpha = 1;
      }
      ...
      for (const qgl of g.quadGhostLenses) {
        ...
        for (const ball of g.balls) {
          ...
          if (dx * dx + dy * dy > QGL_RANGE * QGL_RANGE * 1.44) continue;
      ...
      // ── Axion-string birefringence patchwork (lv302+) ──
      if (g.axionBirePatchwork) {
        const abp = g.axionBirePatchwork;
        for (let ei = 0; ei < abp.edges.length; ei++) {
      ...
      for (const rbh of g.rogueBHs) {
        ...
        const rfx = RBH_LISS_FX * (rbh.fast ? 1.6 : 1);
        const rfy = RBH_LISS_FY * (rbh.fast ? 1.6 : 1);
        const rcx = rbh.cx0 + Math.sin(g.frame * rfx) * RBH_LISS_AX;
        const rcy = rbh.cy0 + Math.sin(g.frame * rfy) * RBH_LISS_AY;
        // instantaneous drift direction (derivative of the parametric position)
        const rvx = Math.cos(g.frame * rfx) * RBH_LISS_AX * rfx;
        const rvy = Math.cos(g.frame * rfy) * RBH_LISS_AY * rfy;
      ...
      // ── FRB delayed old-pop: dark emitter + slow concentric arcs (update + draw) ──
      for (const fd of g.frbDelays) {
        fd.fired = false;
        if (fd.burstAge >= 0) {
          fd.burstAge++;
          if (fd.burstAge > 24) fd.burstAge = -1;
        }
        fd.timer--;
        if (fd.timer <= 0) {
          fd.fired = true;
          fd.burstAge = 0;
          fd.fireAngle = (Math.random() < 0.5 ? -1 : 1) * FRBD_ANGLE;
          fd.timer = fd.period;
        }
        const charging = fd.burstAge < 0 && fd.timer <= FRBD_WARN;
        let sourceOn: boolean;
        if (fd.fired) sourceOn = true;
        else if (charging) {
          const wt = 1 - fd.timer / FRBD_WARN;
          const interval = Math.max(3, Math.round(22 * (1 - wt)) + 3);
          sourceOn = (g.frame % interval) < interval / 2;
        } else {
          sourceOn = (g.frame % 28) < 3;
        }
        ctx.fillStyle = '#a88858';
        if (sourceOn) {
          ctx.globalAlpha = fd.fired ? 0.85 : (charging ? 0.55 : 0.28);
          ctx.fillRect(Math.round(fd.x) - 2, Math.round(fd.y) - 2, 3, 3);
        }
        ctx.globalAlpha = 1;
        if (fd.burstAge >= 0) {
          for (let ring = 0; ring < 2; ring++) {
            const localAge = fd.burstAge - ring * 6;
            if (localAge < 0 || localAge > 16) continue;
            const rt = localAge / 16;
            const rr = rt * FRBD_RING_RANGE;
            for (let i = 0; i < 28; i++) {
              if (i % 4 === 1) continue;
              const a = (i / 28) * Math.PI * 2;
              ctx.globalAlpha = (1 - rt) * 0.45;
              ctx.fillStyle = '#a88858';
              ctx.fillRect(Math.round(fd.x + Math.cos(a) * rr) - 1, Math.round(fd.y + Math.sin(a) * rr) - 1, 2, 2);
            }
          }
          ctx.globalAlpha = 1;
        }
      ...
      if (g.phase === 'firing') {
        ...
        for (let ballIdx = 0; ballIdx < g.balls.length; ballIdx++) {
          ...
          // Interacting HDE transfer: one side axial push (DE->DM toy), other side mild drag.
          for (const ib of g.ihdeBelts) {
            const dy = ball.y - ib.y;
            if (Math.abs(dy) > ib.halfW) continue;
            if (dy >= 0) {
              ball.vx += ib.dir * IHDE_FORCE;
              const isp = Math.sqrt(ball.vx * ball.vx + ball.vy * ball.vy);
              if (isp > BALL_SPEED * 2) { const sc = BALL_SPEED * 2 / isp; ball.vx *= sc; ball.vy *= sc; }
              if (g.frame % 5 === 0) pulseFieldFx(ball, '#905858');
            } else {
              ball.vx *= IHDE_DRAG;
              ball.vy *= IHDE_DRAG;
              if (g.frame % 5 === 0) pulseForceFx(ball, '#687888');
            }
            break;
          }
          ...
          for (const dc of g.dePertCurtains) {
            const ca = Math.cos(dc.angle), sa = Math.sin(dc.angle);
            const dx = ball.x - dc.x, dy = ball.y - dc.y;
            const lx = ca * dx + sa * dy;
            const ly = -sa * dx + ca * dy;
            if (Math.abs(lx) <= DEPERT_LEN * 0.5 && Math.abs(ly) <= DEPERT_THICK * 0.5) {
              dePertScale = DEPERT_GRAV;
              const dTh = DEPERT_TWIST * Math.sin(g.frame * 0.11 + ballIdx);
              const c = Math.cos(dTh), sn = Math.sin(dTh);
              const ovx = ball.vx, ovy = ball.vy;
              ball.vx = ovx * c - ovy * sn;
              ball.vy = ovx * sn + ovy * c;
              if (g.frame % 5 === 0) {
                pulseFieldFx(ball, '#6888a0');
                pulseTwistFx(ball);
              }
              break;
            }
          ...
          for (const bu of g.bubbleUniverses) {
            ...
            if (inside) {
              const gMag = effGrav * BUC_GRAV_SCALE * quintomScale * varCoupScale * h0Scale * s8Scale * nuNullScale * ideSiphonScale * phBeltScale * dePertScale;
          ...
          if (!inBubbleU && !inSpbhEcho) ball.vy += effGrav * quintomScale * varCoupScale * h0Scale * s8Scale * nuNullScale * ideSiphonScale * phBeltScale * dePertScale * scptGravScale;
          const edePreVx = ball.vx;
          ...
          for (const rbh of g.rogueBHs) {
            ...
            const rcx = rbh.cx0 + Math.sin(g.frame * rfx) * RBH_LISS_AX;
            const rcy = rbh.cy0 + Math.sin(g.frame * rfy) * RBH_LISS_AY;
            if (rbh.fast && g.frame % 5 === 0) pulseForceFx(ball, '#c01030');
          ...
          // Spatial curvature keel: edge band slowly steers heading toward/away from center.
          for (const ck of g.curvKeels) {
            const W = g.W, H = g.H;
            const inEdge =
              ball.x < CURVK_BAND || ball.x > W - CURVK_BAND ||
              ball.y < CURVK_BAND || ball.y > H - CURVK_BAND;
            if (!inEdge) continue;
            const desired = Math.atan2(ck.sign * (H * 0.5 - ball.y), ck.sign * (W * 0.5 - ball.x));
            const heading = Math.atan2(ball.vy, ball.vx);
            let diff = desired - heading;
            while (diff > Math.PI) diff -= Math.PI * 2;
            while (diff < -Math.PI) diff += Math.PI * 2;
            const dTh = Math.max(-CURVK_RATE, Math.min(CURVK_RATE, diff));
            const c = Math.cos(dTh), sn = Math.sin(dTh);
            const ovx = ball.vx, ovy = ball.vy;
            ball.vx = ovx * c - ovy * sn;
            ball.vy = ovx * sn + ovy * c;
            if (g.frame % 5 === 0) pulseTwistFx(ball);
            break;
          }
          ...
          // Lyα BAO ghost ring: physical force radius is offset from the visual ring.
          for (const lg of g.lyaGhosts) {
            const ldx = ball.x - lg.x, ldy = ball.y - lg.y;
            const ld2 = ldx * ldx + ldy * ldy;
            if (ld2 === 0) continue;
            const ld = Math.sqrt(ld2);
            const band = Math.abs(ld - lg.rPhys);
            if (band >= LYAG_BAND) continue;
            const lt = 1 - band / LYAG_BAND;
            const lf = LYAG_FORCE * lt * lt * (ld > lg.rPhys ? -1 : 1);
            ball.vx += (ldx / ld) * lf;
            ball.vy += (ldy / ld) * lf;
            if (g.frame % 5 === 0) pulseForceFx(ball, '#807870');
          }
          ...
          // Oblique rare: band Y is tilted; lateral push gains a tilt-aligned bias.
          if (g.cmeY >= 0) {
          ...
          if (g.reionY >= 0) {
            const reionBandY = g.reionY + (ball.x - W * 0.5) * Math.tan(g.reionTilt);
            if (Math.abs(ball.y - reionBandY) < REION_BAND * 0.5) {
              ball.vx *= REION_DRAG_X;
              ball.vy += REION_PUSH_Y;
              if (g.reionTilt !== 0) {
                ball.vx += Math.sin(g.reionTilt) * REION_PUSH_Y * 4;
              }
              const rspd = Math.sqrt(ball.vx * ball.vx + ball.vy * ball.vy);
          ...
          if (g.negNuFlowActive) {
            ...
            if (g.frame % 4 === 0) pulseForceFx(ball, '#8a7060');
          }

          // Ultralight DM undulation: tiny board-wide speed-preserving twist (mean ~0).
          if (g.uldmWaveActive) {
            const dTh = ULDM_AMP * Math.sin(g.frame * ULDM_OMEGA + ballIdx * 1.7);
            const uc = Math.cos(dTh), us = Math.sin(dTh);
            const uvx = ball.vx * uc - ball.vy * us;
            ball.vy = ball.vx * us + ball.vy * uc;
            ball.vx = uvx;
            pulseTwistFx(ball);
          }

          // Flexknot GWB hum: twist frequency jumps at depth thresholds (piecewise spectrum).
          if (g.flexHumActive) {
            const yFrac = ball.y / H;
            let omega = FLEXHUM_W0;
            let band = 0;
            if (yFrac >= 2 / 3) { omega = FLEXHUM_W2; band = 2; }
            else if (yFrac >= 1 / 3) { omega = FLEXHUM_W1; band = 1; }
            if (ball.flexBand !== band) {
              pulseFieldFx(ball, '#586870');
              ball.flexBand = band;
            }
            const dTh = FLEXHUM_AMP * Math.sin(g.frame * omega + ballIdx);
            const fc = Math.cos(dTh), fs = Math.sin(dTh);
            const nvx = ball.vx * fc - ball.vy * fs;
            ball.vy = ball.vx * fs + ball.vy * fc;
            ball.vx = nvx;
            pulseTwistFx(ball);
          }
          ...
          for (const cs of g.cosmicShears) {
            const axisNow = cs.orthoFlip
              ? cs.axis + (Math.floor(g.frame / 120) % 2) * (Math.PI / 2)
              : cs.axis;
            const ca = Math.cos(axisNow), sa = Math.sin(axisNow);
            ...
            const heading = Math.atan2(ball.vy, ball.vx);
            const dTheta = -CSHEAR_ROT * Math.sin(2 * (heading - axisNow)) * ct * ct;
          ...
          // Baryonic feedback softening: continuous speed damp (heading unchanged).
          for (const bs of g.barySofts) {
            const ca = Math.cos(bs.axis), sa = Math.sin(bs.axis);
            const dx = ball.x - bs.x, dy = ball.y - bs.y;
            const u = (ca * dx + sa * dy) / bs.rx;
            const v = (-sa * dx + ca * dy) / bs.ry;
            if (u * u + v * v >= 1) continue;
            ball.vx *= BARYSOFT_DRAG;
            ball.vy *= BARYSOFT_DRAG;
            const spd = Math.sqrt(ball.vx * ball.vx + ball.vy * ball.vy);
            if (spd > 1e-8 && spd < effMinSpeed) {
              const sc = effMinSpeed / spd;
              ball.vx *= sc; ball.vy *= sc;
            }
            if (g.frame % 5 === 0) pulseForceFx(ball, '#786858');
          }
          ...
          for (const silk of g.silkDampingClouds) {
            ...
            if (silk.longAxis) {
              const newAlong = along * SILK_ACROSS;
              ball.vx = newAlong * ca - across * sa;
              ball.vy = newAlong * sa + across * ca;
            } else {
            ...
            const dampComp = silk.longAxis ? along : across;
            if (Math.abs(dampComp) > 0.08 && g.frame % 4 === 0) {
              ball.fxTrail = 3;
          ...
          for (const cb of g.cosmicBirefringences) {
            ...
            if (ball.bfSide === 0) {
              ...
              const cbSign = cb.inverted ? -1 : 1;
              const cbRot = (ball.bfSide === 1 ? CB_ROT : -CB_ROT) * cbSign; // front(1)->back: +0.22; back->front(1): -0.22
          ...
          // Magnetar birth chirp: same outward shove, plus a chirping Lense-Thirring twist.
          for (const bc of g.birthChirps) {
            if (bc.releaseTimer <= 0) continue;
            const bdx = ball.x - bc.x, bdy = ball.y - bc.y;
            const bd2 = bdx * bdx + bdy * bdy;
            if (bd2 >= MBC_RANGE * MBC_RANGE || bd2 === 0) continue;
            const bd = Math.sqrt(bd2);
            const bt = 1 - bd / MBC_RANGE;
            const bf = MBC_FORCE * bt * bt;
            ball.vx += (bdx / bd) * bf;
            ball.vy += (bdy / bd) * bf;
            const twist = Math.min(MBC_TWIST_CAP, MBC_TWIST0 + bc.chirpN * MBC_TWIST_STEP) * bc.twistSign;
            const c = Math.cos(twist), sn = Math.sin(twist);
            const ovx = ball.vx, ovy = ball.vy;
            ball.vx = ovx * c - ovy * sn;
            ball.vy = ovx * sn + ovy * c;
            pulseForceFx(ball, '#ffe8a0');
            pulseTwistFx(ball);
          }

          // GWB-LSS anisotropic bias: push along fixed axis; weight from heading quadrupole.
          for (const gb of g.gwbLssBiases) {
            const heading = Math.atan2(ball.vy, ball.vx);
            const ang = heading - gb.theta0;
            const w = 0.35 + 0.65 * Math.max(0, Math.cos(2 * ang)); // HD-like quadrupole lobe
            const f = gb.accel * w;
            ball.vx += Math.cos(gb.theta0) * f;
            ball.vy += Math.sin(gb.theta0) * f;
            const gspd = Math.sqrt(ball.vx * ball.vx + ball.vy * ball.vy);
            if (gspd > BALL_SPEED * 2) {
              const sc = BALL_SPEED * 2 / gspd;
              ball.vx *= sc;
              ball.vy *= sc;
            }
            if (f > 0.008) pulseFieldFx(ball, '#6a5890');
          }

          // JWST SFE overproduction: mild isotropic boost + weak outward (never traps).
          for (const sf of g.sfeOvers) {
            const ca = Math.cos(sf.axis), sa = Math.sin(sf.axis);
            const dx = ball.x - sf.x, dy = ball.y - sf.y;
            const u = (ca * dx + sa * dy) / sf.rx;
            const v = (-sa * dx + ca * dy) / sf.ry;
            if (u * u + v * v >= 1) continue;
            ball.vx *= SFE_BOOST;
            ball.vy *= SFE_BOOST;
            const d2 = dx * dx + dy * dy;
            if (d2 > 1) {
              const d = Math.sqrt(d2);
              ball.vx += (dx / d) * SFE_OUT;
              ball.vy += (dy / d) * SFE_OUT;
            }
            const spd = Math.sqrt(ball.vx * ball.vx + ball.vy * ball.vy);
            if (spd > BALL_SPEED * 2) { const sc = BALL_SPEED * 2 / spd; ball.vx *= sc; ball.vy *= sc; }
            else if (spd > 0 && spd < SFE_MIN_SPD) { const sc = SFE_MIN_SPD / spd; ball.vx *= sc; ball.vy *= sc; }
            pulseForceFx(ball, '#c8c0b0');
          }

          // Persistent radio nebula: isotropic drag + weak outward (never traps).
          for (const prs of g.prsNebulae) {
            const pdx = ball.x - prs.x, pdy = ball.y - prs.y;
            const pd2 = pdx * pdx + pdy * pdy;
            if (pd2 >= PRS_R * PRS_R || pd2 === 0) continue;
            const pd = Math.sqrt(pd2);
            ball.vx *= PRS_DRAG;
            ball.vy *= PRS_DRAG;
            ball.vx += (pdx / pd) * PRS_OUT;
            ball.vy += (pdy / pd) * PRS_OUT;
            const pspd = Math.sqrt(ball.vx * ball.vx + ball.vy * ball.vy);
            if (pspd > 0 && pspd < PRS_MIN_SPD) {
              const sc = PRS_MIN_SPD / pspd;
              ball.vx *= sc;
              ball.vy *= sc;
            }
            pulseForceFx(ball, '#3a9a78');
          }

          // Dimming DM shell: grow-phase band only — soft drag + weak outward normal refract.
          for (const ds of g.dimDmShells) {
            if (ds.phase !== 'grow') continue;
            const ddx = ball.x - ds.x, ddy = ball.y - ds.y;
            const dd2 = ddx * ddx + ddy * ddy;
            if (dd2 === 0) continue;
            const dd = Math.sqrt(dd2);
            if (Math.abs(dd - ds.radius) >= DDS_BAND_HALF) continue;
            ball.vx *= DDS_DRAG;
            ball.vy *= DDS_DRAG;
            const t = 1 - Math.abs(dd - ds.radius) / DDS_BAND_HALF;
            const f = DDS_REFRACT * t * t;
            ball.vx += (ddx / dd) * f;
            ball.vy += (ddy / dd) * f;
            pulseFieldFx(ball, '#687070');
            const dang = Math.atan2(ddy, ddx);
            const binWidth = (Math.PI * 2) / DDS_LIT_BINS;
            const dbin = (Math.round(dang / binWidth) % DDS_LIT_BINS + DDS_LIT_BINS) % DDS_LIT_BINS;
            ds.litBins[dbin] = DDS_LIT_DUR;
          }

          // RM flare corridor: one speed-preserving twist the first frame a ball sits in the band.
          for (const rf of g.rmFlares) {
            if (rf.pos < 0) continue;
            const along = ball.x * rf.tx + ball.y * rf.ty;
            if (Math.abs(along - rf.pos) > RMF_HALF + BALL_R) continue;
            if (rf.hit.has(ball)) continue;
            rf.hit.add(ball);
            const ang = RMF_ANGLE * rf.twistSign;
          ...
          // Pop III.1 Flash:
          // Memory-burden ember: weak continuous pull; young-evaporation outward pulse while releasing.
          for (const mbe of g.memoryBurdenEmbers) {
            const mdx = ball.x - mbe.x, mdy = ball.y - mbe.y;
            const md2 = mdx * mdx + mdy * mdy;
            if (md2 > 0 && md2 < MBE_RANGE * MBE_RANGE) {
              const md = Math.sqrt(md2);
              const mt = 1 - md / MBE_RANGE;
              const mf = MBE_PULL * mt * mt;
              ball.vx -= (mdx / md) * mf;
              ball.vy -= (mdy / md) * mf;
              if (g.frame % 5 === 0) pulseFieldFx(ball, '#8a8078');
            }
            if (mbe.releaseTimer > 0 && md2 > 0 && md2 < MBE_PULSE_RANGE * MBE_PULSE_RANGE) {
              ...
              if (spd > BALL_SPEED * 2) {
                ...
                ball.vy *= (BALL_SPEED * 2) / spd;
              }
            }
          }

          // Pop III.1 Flash:
          // Hostless LRD seed: tangential Keplerian swirl + weak inward (no absorb).
          for (const ns of g.nakedLrdSeeds) {
            const dx = ball.x - ns.x, dy = ball.y - ns.y;
            const d2 = dx * dx + dy * dy;
            if (d2 === 0 || d2 >= NLS_RANGE * NLS_RANGE) continue;
            const d = Math.sqrt(d2);
            const t = 1 - d / NLS_RANGE;
            const ux = dx / d, uy = dy / d;
            const tx = -uy, ty = ux;
            const ft = NLS_TANG * t * t;
            const fi = NLS_IN * t * t;
            ball.vx += tx * ft - ux * fi;
            ball.vy += ty * ft - uy * fi;
            pulseTwistFx(ball);
            if (d < 40) pulseFieldFx(ball, '#a03028');
            const spd = Math.hypot(ball.vx, ball.vy);
            if (spd > BALL_SPEED * 2) {
              ball.vx *= (BALL_SPEED * 2) / spd;
              ball.vy *= (BALL_SPEED * 2) / spd;
          ...
          // Dressed PBH halo: weak continuous pull (cloak invisible).
          for (const dp of g.dressedPbhs) {
            const dx = dp.x - ball.x, dy = dp.y - ball.y;
            const d2 = dx * dx + dy * dy;
          ...
          // F_AP anisotropy loom: axis stretch / ortho compress, then renormalize speed.
          for (const fl of g.fapLooms) {
            const dx = ball.x - fl.x, dy = ball.y - fl.y;
            const c = Math.cos(fl.axis), sn = Math.sin(fl.axis);
            const lx = c * dx + sn * dy;
            const ly = -sn * dx + c * dy;
            if ((lx * lx) / (fl.rx * fl.rx) + (ly * ly) / (fl.ry * fl.ry) > 1) continue;
            const spd0 = Math.hypot(ball.vx, ball.vy);
            if (spd0 < 1e-6) continue;
            const ax = c, ay = sn;
            const ox = -sn, oy = c;
          ...
          for (const ch of g.chameleons) {
            const cdx = ball.x - ch.x, cdy = ball.y - ch.y;
            const cd2 = cdx * cdx + cdy * cdy;
            if (cd2 >= CHAM_RANGE * CHAM_RANGE || cd2 < 1) continue;
            let n = 0;
            const nR2 = CHAM_NEIGHBOR * CHAM_NEIGHBOR;
            for (const peg of g.pegs) {
              if (peg.cleared || peg.mudBroken) continue;
            ...
            const ct = 1 - cd / CHAM_RANGE;
            const cf = (CHAM_FORCE * ct * ct) / (1 + CHAM_SCREEN * n);
            // sign +1 = outward (from center to ball), -1 = inward
            ball.vx += (cdx / cd) * cf * ch.sign;
            ball.vy += (cdy / cd) * cf * ch.sign;
            if (cf > 0.01 && g.frame % 6 === 0) pulseFieldFx(ball, '#687868');
          }
          ...
          // FRB delayed old-pop: same board-wide twist, milder angle, long telegraph.
          for (const fd of g.frbDelays) {
            if (!fd.fired) continue;
            const fca = Math.cos(fd.fireAngle), fsa = Math.sin(fd.fireAngle);
          ...
          // Quadruple-image ghost lens: weak inward pull; exit after closest approach kicks outward.
          for (const qgl of g.quadGhostLenses) {
            const dx = qgl.x - ball.x, dy = qgl.y - ball.y;
            const dist = Math.sqrt(dx * dx + dy * dy);
            const inside = dist < QGL_RANGE && dist > 1e-6;
            let st = qgl.track.get(ball);
            if (inside) {
          ...
          // Axion-string birefringence patchwork: in-strip micro-twist + boundary dump.
          if (g.axionBirePatchwork) {
            const abp = g.axionBirePatchwork;
            let strip = 0;
            while (strip < abp.edges.length && ball.x >= abp.edges[strip]) strip++;
            const prev = abp.lastStrip.get(ball);
            if (prev !== undefined && prev !== strip) {
              const dTh = (strip > prev ? 1 : -1) * ABP_CROSS * abp.signs[strip];
              const bc = Math.cos(dTh), bs = Math.sin(dTh);
              const nvx = ball.vx * bc - ball.vy * bs;
              ball.vy = ball.vx * bs + ball.vy * bc;
              ball.vx = nvx;
          ...
          for (const md of g.measDisagreeDuals) {
            const dx = ball.x - md.x, dy = ball.y - md.y;
            const c = Math.cos(md.axis), sn = Math.sin(md.axis);
            const lx = c * dx + sn * dy;
            const ly = -sn * dx + c * dy;
            if ((lx * lx) / (md.rx * md.rx) + (ly * ly) / (md.ry * md.ry) > 1) continue;
            const phase = Math.sin(g.frame * MEASDUAL_OMEGA);
          ...
          // EB parity fault: opposite micro-twist on left/right; midline dump on cross.
          if (g.ebParityActive) {
            const mid = W * 0.5;
          ...
          // never skips over thin collision zones (bumpers, wormhole bars).
          {
            ...
            for (let sub = 0; sub < substeps; sub++) {
              ...
              if (!teleported) for (const cls of g.collisionlessShocks) {
                ...
                const hang = Math.atan2(cls.vy, cls.vx);
                const armBase = cls.inverted ? hang : hang + Math.PI;
                const armAngles = [armBase - cls.armSpread, armBase + cls.armSpread];
                let nearAny = false;
              ...
              // Dressed PBH caustic ring: one-shot radial kick + twist.
              if (!teleported) for (const dp of g.dressedPbhs) {
                const mdx = ball.x - dp.x, mdy = ball.y - dp.y;
                const mdist = Math.sqrt(mdx * mdx + mdy * mdy) || 1;
                const band = Math.abs(mdist - DPBH_RING_R);
                const inside = band < DPBH_RING_HALF + BALL_R;
                if (!inside) { dp.passingBalls.delete(ball); continue; }
                if (dp.passingBalls.has(ball)) continue;
                dp.passingBalls.add(ball);
                const nx = mdx / mdist, ny = mdy / mdist;
                ball.vx += nx * DPBH_KICK;
                ball.vy += ny * DPBH_KICK;
                const tw = ((Math.floor(ball.x) ^ Math.floor(ball.y)) & 1) === 0 ? DPBH_TWIST : -DPBH_TWIST;
                ...
                dp.flashTimer = DPBH_FLASH;
                pulseTwistFx(ball);
                pulseForceFx(ball, '#c8b090');
              }

              // Supercooled PT wall: one-shot outward kick on band cross.
              if (!teleported) for (const sw of g.scptWalls) {
                if (sw.respawnTimer > 0) continue;
                const mdx = ball.x - sw.x, mdy = ball.y - sw.y;
                const mdist = Math.sqrt(mdx * mdx + mdy * mdy) || 1;
                const band = Math.abs(mdist - sw.r);
                const inside = band < SCPT_HALF + BALL_R;
                if (!inside) { sw.passingBalls.delete(ball); continue; }
              ...
              // Free-streaming cutoff blade: zero small normal velocity on one-shot cross.
              if (!teleported && g.fsCutoffBlade) {
                const bl = g.fsCutoffBlade;
                const d = bl.horizontal ? Math.abs(ball.y - bl.pos) : Math.abs(ball.x - bl.pos);
                if (d <= FSBLADE_HALF + BALL_R) {
                  if (!bl.passingBalls.has(ball)) {
  ...
  useEffect(() => {
    ...
    (window as unknown as { __dotshotInfo?: () => Record<string, unknown> }).__dotshotInfo = () => {
      ...
      const counts: Record<string, number | boolean | null> = {
        ...
        barySofts: g.barySofts.length,
        chameleons: g.chameleons.length,
        lyaGhosts: g.lyaGhosts.length,
        flexHum: g.flexHumActive,
        uldmWave: g.uldmWaveActive,
        birthChirps: g.birthChirps.length,
        rmFlares: g.rmFlares.length,
        dimDmShells: g.dimDmShells.length,
        prsNebulae: g.prsNebulae.length,
        gwbLssBiases: g.gwbLssBiases.length,
        sfeOvers: g.sfeOvers.length,
        dePertCurtains: g.dePertCurtains.length,
        negNuFlowActive: g.negNuFlowActive ? 1 : 0,
        frbDelays: g.frbDelays.length,
        curvKeels: g.curvKeels.length,
        ihdeBelts: g.ihdeBelts.length,
