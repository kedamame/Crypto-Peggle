...
目的: **新カタログ #48-53 を codex がゼロから起草・実装する**ための、作業に一切支障が出ない引き継ぎ。

---

## 0. まず読む順

1. `CLAUDE.md` — このリポジトリ専用ガイドライン。**§1 North Star / §2 ビジュアル契約 / §3 進行表 /
   §4 進行原則 / §6 8タッチポイント / §7 物理エンジンの仕組み**。ここが正典。
2. `docs/COSMIC_GIMMICK_SPEC.md` — §1 共通実装レシピ / §2 深度マップ＋カタログ #1-47（**完走済み**）/
   §3 重複リスク表 / §4 採否ログ。
3. `docs/GIMMICK_DESIGN_GUIDE.md` — Tier 視認性基準・色ファミリーマップ・§5 改修ログ。
4. このファイル（現状・配線アンカー・今セッションの落とし穴）。
5. `HANDOFF_GROK.md`（履歴。#1-47 完走の記録と旧教訓）。

すべて日本語で書かれている。新規ドキュメントも日本語で揃えること。
...
- **カタログ #1-47 は完走・実装済み**（`docs/COSMIC_GIMMICK_SPEC.md` §4 採否ログが正）。
- 直近セッション（Opus 4.8）で以下を追加・push 済み。最新 `master` は revert 後の状態:
  - **Batch I**（`64cbf13`）: 挙動バリアント前半 — 連星彗星 lv45 / 往復赤彗星 lv50 / 真空バブル対 lv52 / 脈動BH lv60。
  - **Batch J**（`b011a9e`）: 挙動バリアント後半 — ワームホール三連 lv47 / 二重逆回転レンズ lv48 /
    三条パルサー lv54 / 漂うマグネター lv63。
  - **Batch K**（`51d9ad2`）: 新ペグ型 — 中性子 lv28 / 対生成 lv31 / 量子もつれ lv34 / 赤方偏移 lv38。
  - Batch L で一度 #48-53 のドラフトを書いたが、**ユーザー判断で白紙化（revert `be6f7e3`）**。
    → **#48-53 の内容・レベル・現象は未定。codex が自由に起草してよい。**

### codex のタスク: #48-53 を起草・実装する

- **現象の選定は codex に一任**。ただし §4 の進行原則（下記）と §2 ビジュアル契約を満たすこと。
- レベル帯: 深度マップ（`COSMIC_GIMMICK_SPEC.md` §2.0）は #1-47 で lv23〜99 を既に埋めているが、
  **1レベル帯に複数ハザードが解禁されてよい**（確率ロールで散らばる）。#48-53 は既存ゾーンに
  「密度を足す」新現象として差し込む想定。深いほど exotic（原則 lv22+、特に深部ゾーンC/D/E）。
- **記入先3ファイル**（実装したら必ず全て更新）:
  1. `COSMIC_GIMMICK_SPEC.md` §4 採否ログに1行（`✅実装 | Lv | 要点`）。ドラフトを §5 に書くなら新設可。
  2. `CLAUDE.md` §3 進行表 or §5 ログ表に1行。
  3. `docs/GIMMICK_DESIGN_GUIDE.md` §5 改修ログにコミットID入りで1行。
