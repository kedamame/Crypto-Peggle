...
> ### ゾーンM: 誕生と散乱の前線（lv240〜259）— 天体誕生と電波散乱が盤面をねじる
> マグネター誕生チャープ・RMフレア回廊・減光DM殻・持続電波星雲・GWB–LSS異方・超軽量DMうねり。
> **動きのトーン**: k≤0.0012。パルス増ねじれ・回廊回転・膨張殻・弱い場・角度依存・全域微ねじれ。
> **形のトーン**: 氷琥珀フレア・帯状回廊・薄い殻・星雲暈・異方流線・うねり格子。**玉の変質が主tell**。
...
## 13. 新カタログ #102〜#107（ゾーンM / 誕生と散乱の前線）

> 2026-07-14 起草。#96-101（ゾーンL）完了後の **Lv240〜259**。
> モチーフは SN 2024afav マグネター誕生チャープ・FRB RM flare・SNR減光殻・持続電波星雲・PTA×LSS・ULDM。
...
- **元ネタ**: SN 2024afav / Nature 2026 — 誕生直後マグネターの Lense–Thirring チャープ。
- **出現**: 35%。`magnetars.length === 0`（通常マグネターと排他）。
- **物理**: マグネター同型の予告→放出。放出中 `R=155` で外向き `f=1.35*t²` ＋速度保存ねじれ。
  ねじれ角 = `min(0.24, 0.07 + chirpN*0.028) * twistSign`。`chirpN` は各パルス終了後に +1。
...
- **出現**: 35%。`silkDampingClouds`空。
- **物理**: 固定円 R=90。内部 v*=0.988（角度保存）+微外向 0.05。通過可・吸収なし。速度床 PRS_MIN_SPD。
- **玉FX**: Force trail `#3a9a78`（場内で毎フレーム）。
- **ビジュアル**: 電波緑の淡いハロー＋遅い点滅核。
- **実装メモ**: `prsNebulae: PrsNebula[]`。定数 `PRS_*`。

### 13.106 GWB–LSS異方バイアス — GWB–LSS Anisotropic Bias（Lv目安 254）

- **元ネタ**: PTA GWB × 大規模構造クロス相関（JCAP 2026 / arXiv:2603.02110）。
- **出現**: 35%。hdHum/alens/gwb/flexHum/blueHum/isoBire/gravWaves 排他。
- **物理**: 全域。固定軸 theta0 方向へ加速。強さ = accel * (0.35+0.65*max(0,cos(2*(heading-theta0))))。BALL_SPEED*2 クランプ。
- **玉FX**: Field `#6a5890`。
...
- **ビジュアル**: 暗欠け弧＋潜熱琥珀スパーク。
- **実装メモ**: `scptWalls: ScptWall[]`。定数 `SCPT_*`。サブステップ衝突必須。ゾーンO完走（`57f3caa`）。

---

## 16. 新カタログ #120〜#125（ゾーンP / 観測が盤面を食い破る）

> 2026-07-22 再起草。旧案（フィラメント／自由流霧／EDE微ブースト／Bモード囁き等）は面白さテストで却下。
> #114-119（ゾーンO）完了後の **Lv300〜319**。
> モチーフはアクシオン弦複屈折パッチ・四重像レンズ・EDE法則点滅・自由流カットオフ刃・EBパリティ断層・測定不一致二重場。

### 16.0 早見

| # | 名称 | Lv | 分類 | rng | ロール | 排他 |
|---|---|---|---|---|---|---|
| 120 | アクシオン弦複屈折パッチワーク | 302 | 帯内微ねじれ＋境界横断ねじれ | `abpRng` | 40% | isoBire / cosmicBire / ebParity |
| 121 | 四重像ゴーストレンズ | 305 | 弱引力＋退出キック＋描画ゴースト | `qglRng` | 40% | dressedPbh / einsteinCross |
| 122 | EDE法則点滅 | 308 | 予告→10f連続フォース符号反転 | `edeBlinkRng` | 35% | quintom / signIde |
| 123 | 自由流カットオフ刃 | 311 | 掃引刃・低速法線成分切断 | `fsBladeRng` | 40% | silk / CME |
| 124 | EBパリティ断層 | 314 | 左右逆微ねじれ＋中央断層キック | `ebParityRng` | 40% | abp / isoBire |
| 125 | 測定不一致二重場 | 317 | 楕円内ねじれ⇔外向力の位相切替 | `measDualRng` | 35% | shear / fapLoom |

**rng**: `scptWallRng` → `abpRng` → `qglRng` → `edeBlinkRng` → `fsBladeRng` → `ebParityRng` → `measDualRng`

**ゾーンP 色**: 紫`#7a5a98` / シアン`#5a98a8` / 暗琥珀`#a87840` / 四重像ゴースト`#c8b090` / EDE琥珀`#d8a860` / カットオフコバルト`#4060a0` / 剪断金`#c8a060` / クラスタ灰`#888880`

> ### ゾーンP: 観測が盤面を食い破る（lv300〜319）
> 弦の影が空を割る／四重像が玉を分身させる／EDEが一瞬だけ力の符号を裏返す／自由流長が細部を削ぐ／パリティが左右で食い違う／二つの測定が同じ玉で喧嘩する。
> **動きのトーン**: 境界イベント・掃引・短点滅・位相切替。弱い連続場だけでは終わらせない。
> **形のトーン**: パッチ縫い目・分身ゴースト・四隅予告・薄い刃・左右ダスト・二重輪郭。**玉FXが主tell**。

### 16.120 アクシオン弦複屈折パッチワーク — Axion-String Birefringence Patchwork（Lv目安 302）

- **元ネタ**: アクシオン弦による CMB 複屈折の非ガウス・パッチワーク。
- **出現**: 40%。!isoBire / cosmicBirefringences空 / !ebParityActive。
- **物理**: 垂直ストリップ3〜5枚。帯内 `±0.004rad/f`（速度保存）。内部境界横断1回で `±0.20rad`（WeakMapでストリップ追跡）。
- **玉FX**: 帯内 Twist；境界で強い Twist。
- **ビジュアル**: 境界1pxドット線（紫／シアン交互）。内部ほぼ無描画。
- **実装メモ**: `axionBirePatchwork: AxionBirePatchwork | null`。定数 `ABP_*`。

### 16.121 四重像ゴーストレンズ — Quadruple-Image Ghost Lens（Lv目安 305）
...
- **元ネタ**: 暖／熱いDMの自由流長 λ_FS。
- **出現**: 40%。silkDampingClouds空 / !cmeActive。
- **物理**: 垂直（または水平）半幅6の刃が端→端掃引。横断時 `|vn|<0.55` なら法線成分を0（接線維持）。WeakSet 1横断1回。
- **玉FX**: Force trail `#4060a0`。
- **ビジュアル**: コバルト1〜2px刃＋短い欠落ノイズ。霧は描かない。
- **実装メモ**: `fsCutoffBlade: FsCutoffBlade | null`。定数 `FSBLADE_*`。サブステップ必須。

### 16.124 EBパリティ断層 — EB Parity Fault（Lv目安 314）

- **元ネタ**: CMB EB パリティ破れ。
- **出現**: 40%。axionBirePatchwork===null / !isoBire。
- **物理**: 左半面 `+0.006rad/f`、右 `-0.006`。中央 `|x-W/2|<10` を横断した瞬間 `±0.22rad` 1回（Ball.ebSideで検知）。
- **玉FX**: 半面内 Twist；断層で強い Twist。
- **ビジュアル**: 左縁紫ダスト列、右縁シアン、中央1px食い違い線。
- **実装メモ**: `ebParityActive: boolean`。定数 `EBPAR_*`。Ball.ebSide。

### 16.125 測定不一致二重場 — Measurement-Disagreement Dual Field（Lv目安 317）
...
- **実装メモ**: `hpmfLorCorridors: HpmfLorCorridor[]`。定数 `HPMF_*`。実装済（`b8fdb15`）。
...
- **ビジュアル**: 銅赤輝線＋波長ティック。
- **実装メモ**: `axionIrLines: AxionIrLine[]`。定数 `AXIR_*`。実装済（`5337241`）。
...
- **ビジュアル**: 銀灰欠け環が縮む。
- **実装メモ**: `rsShrinkScars: RsShrinkScar[]`。定数 `RSSH_*`。実装済（`5337241`）。
...
- **ビジュアル**: 銀藍の欠け円錐前線のみ。
- **実装メモ**: `ectHorizons: EctHorizon[]`。定数 `ECT_*`。実装済（`98bece1`）。
...
- **ビジュアル**: 錆金欠け壁＋誘起二次弧。
- **実装メモ**: `dwInducedWalls: DwInducedWall[]`。定数 `DWIND_*`。実装済（`98bece1`）。
...
## 20. 新カタログ #144〜#149（ゾーンT / 遅延した信号と食い違う供給）

> 2026-07-24 起草。#138-143（ゾーンS）完了後の **Lv380〜399**。
> テーマは Zone S「変換と階層」と被らない **「遅延した信号と食い違う供給」** —
> 協力放出の遅れ・フィラメント給餌・二重ファントム横断・弦スペクトルの谷・標準サイレン距離・RMコヒーレンス。
> 却下（重複）: audible axion 再包装、二重再電離再出、ピーナッツ面、LRD/Thomson再色、全球ΔN_eff。

### 20.0 早見

| # | 名称 | Lv | 分類 | rng | ロール | 排他 |
|---|---|---|---|---|---|---|
| 144 | 超蛍光アクシオン雲パルス | 382 | 長予告→遅延外向きバースト | `sfAxionRng` | 40% | birthChirp / audibleAx / hawking（当選時 magnetars 空化） |
| 145 | CND給餌フィラメント渦 | 385 | ポリライン接線ドラッグ＋中心加速 | `cndFeedRng` | 40% | laniakea / gts |
| 146 | 二重ファントム横断帯 | 388 | 横断で w 符号反転（外/内切替） | `dualPhanRng` | 35% | phantom膜/帯 / dissipDe / ideSiphon / DE域 |
| 147 | フレーバー弦プラトー谷 | 391 | 外プラトー弱外向＋谷帯ねじれ | `flavStrRng` | 40% | GW / ORC / twinPeak / alpEcho / ECT |
| 148 | 標準サイレン距離断層 | 394 | 刃横断で進行方向±18pxシフト | `stdSirenRng` | 35% | cosmicStrings / photoZ |
| 149 | RMコヒーレンス回廊 | 397 | 帯内 Faraday 接線ねじれ | `rmCohRng` | 40% | hpmfLor |

**rng**: `alpEchoRng` → `sfAxionRng` → `cndFeedRng` → `dualPhanRng` → `flavStrRng` → `stdSirenRng` → `rmCohRng`

**ゾーンT 色**: 超蛍光黄緑`#c8e0a0` / 給餌錆桃`#a87888` / ファントム桃`#d0a0a8`・錆氷`#7a8890` / 弦錆灰`#8a7880` / サイレン銀藍`#687898` / RMシアン`#4a8898`

> ### ゾーンT: 遅延した信号と食い違う供給（lv380〜399）
> 超蛍光遅延・給餌フィラメント・二重ファントム横断・弦プラトー谷・標準サイレン断層・RM回廊。
> **動きのトーン**: 長い予告→遅延発火、フィラメント収束、横断切替、プラトー/谷の二層、瞬間距離ジャンプ、コヒーレントねじれ。
> **形のトーン**: 欠け環・点列フィラメント・二色帯・刃。閉輪郭を避ける。α≥0.30。**玉FXが主tell**。

### 20.144 超蛍光アクシオン雲パルス — Superradiant Axion Cloud Burst（Lv目安 382）

- **元ネタ**: 超放射雲の重力超蛍光（arXiv:2606.10776）。
- **出現**: 40%。birthChirps / audibleAxLattices / hawkingPoints 空。当選時は `magnetars` を空化してフレア言語を一本化（深部でマグネター50%に食い潰されない）。
- **物理**: 予告≥36f → 放出8fで範囲 `R=150` 内に外向き `f≥0.55*t*t`（マグネター型だが **予告が長く・放出が遅れて一気**）。平常時無力。
- **玉FX**: Force `#c8e0a0`＋Trail。
- **ビジュアル**: 薄い核＋遅れて爆発する欠け衝撃波（歳差ギャップ）。α≥0.32。
- **実装メモ**: `sfAxionClouds`。定数 `SFAX_*`。timer/releaseTimer。fun-fixでマグネター置換。

### 20.145 CND給餌フィラメント渦 — CND Feeding Filament Swirl（Lv目安 385）

- **元ネタ**: JWST NGC4696 多相フィラメント→周核円盤。
- **出現**: 40%。laniakeaBasins / galacticTidalStreams 空。
- **物理**: 盤外から中心へ向かうポリライン帯（`closestOnPolyline`）で接線ドラッグ `f=0.28*t*t`。中心近傍（dist<50）で接線加速+0.12。**シンク引力なし**。
- **玉FX**: Force 接線 `#a87888`。
- **ビジュアル**: 錆桃フィラメント点列＋中心S字欠け渦（閉輪禁止）。α≥0.30。
- **実装メモ**: `cndFeedFils`。定数 `CNDF_*`。ラニアケア流用・シンク引力ゼロを単体simで確認。

### 20.146 二重ファントム横断帯 — Dual Phantom Crossing Belt（Lv目安 388）

- **元ネタ**: 相互作用DE二重横断（arXiv:2605.20060）。
- **出現**: 35%。phantomMembranes / phantomBelts / dissipDeWakes / ideSiphonBands / darkEnergyPatches 空。
- **物理**: 傾いた帯（半幅22）。横断のたび `w` 符号反転（外向0.35 / 内向0.28）。WeakMap側追跡。1横断1切替。
- **玉FX**: Trail 桃⇄錆氷（符号で色反転必須）。
- **ビジュアル**: 二色点線帯が位相ずれで明滅。α≥0.32。
- **実装メモ**: `dualPhantomBelts`。定数 `DPHAN_*`。`lastSide` WeakMap。

### 20.147 フレーバー弦プラトー谷 — Flavored String Plateau–Valley（Lv目安 391）

- **元ネタ**: flavored axion 弦GWの plateau–valley（arXiv:2606.05320）。
- **出現**: 40%。gravWaves / ORC / twinPeakShells / alpEchoShells / ectHorizons 空。
- **物理**: 同心2帯。外プラトー（r≈90±14）弱外向 `f=0.22*t*t`。谷帯（r≈55±10）は速度保存微ねじれ ±0.035rad/f。永続（生滅なし）。
- **玉FX**: Force／Twist `#8a7880`。
- **ビジュアル**: 欠け二重環＋谷だけ暗い欠落。α≥0.30。
- **実装メモ**: `flavStrValleys`。定数 `FLAVSTR_*`。リング族排他。

### 20.148 標準サイレン距離断層 — Standard-Siren Distance Fault（Lv目安 394）

- **元ネタ**: DESI+将来GW標準サイレン距離緊張。
- **出現**: 35%。cosmicStrings / photoZGates 空。
- **物理**: 傾いた薄い刃（半幅4）横断で位置を進行方向へ ±18px シフト（速度不変・宇宙ひも系）。WeakSet 1横断1回。**サブステップ必須**。
- **玉FX**: Field `#687898`＋残像。
- **ビジュアル**: 銀藍欠け刃＋新旧位置1f残像。α≥0.34。
- **実装メモ**: `stdSirenFaults`。定数 `STDSIR_*`。revive WeakSet。

### 20.149 RMコヒーレンス回廊 — RM Coherence Corridor（Lv目安 397）

- **元ネタ**: A&A 2026 PMF RM天空構造。
- **出現**: 40%。hpmfLorCorridors 空。
- **物理**: 幅狭い帯（半幅16）内で Faraday 風＝接線ねじれ連続（±0.028rad/f・速さ保存）。帯外無力。
- **玉FX**: Twist `#4a8898`。
...
- **実装メモ**: `chirpEchoes`。定数 `CHIRPECHO_*`。`f97f0b8`。
...
- **実装メモ**: `gzRibbons`。定数 `GZRIB_*`。`f97f0b8`。
...
- **実装メモ**: `reionOverlaps`。定数 `REIONOLAP_*`。`f97f0b8`。
...
- **実装メモ**: `dlaCurtains`。定数 `DLACURT_*`。`f97f0b8`。
...
- **実装メモ**: `axStringBundles`。定数 `AXSTRB_*`。`f97f0b8`。
